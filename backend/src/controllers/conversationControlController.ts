...
function safe(value: import("../conversation/domain/conversationControl.js").ConversationControl, actorId: string) { return { controlState: value.state, controlledByCurrentActor: value.state === "human_controlled" && value.controllingActorId === actorId, attentionReason: value.attentionReason, takenAt: value.takenAt, releasedAt: value.releasedAt, lastOperatorActivityAt: value.lastOperatorActivityAt, resolvedAt: value.resolvedAt, controlVersion: value.version, authorityGeneration: value.authorityGeneration, updatedAt: value.updatedAt }; }
