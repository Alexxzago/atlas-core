...
export function GuidedSignIn(): React.JSX.Element {
  const { t } = useI18n(); const { state: auth, login } = useAuthentication();
  ...
  return <Surface className="guided-registration" tone="raised"><form onSubmit={(event) => void signIn(event)}><Stack gap="4"><h1 ref={heading} tabIndex={-1}>{t("assistantSetup.signIn.title")}</h1><p>{t("assistantSetup.signIn.description")}</p><label>{t("registration.field.email")}<input autoComplete="username" required type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label>{t("registration.field.password")}<input autoComplete="current-password" required type="password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>{error && <p role="alert">{error}</p>}<Button type="submit" disabled={loading}>{t(loading ? "assistantSetup.loading" : "assistantSetup.continue")}</Button></Stack></form></Surface>;
}
