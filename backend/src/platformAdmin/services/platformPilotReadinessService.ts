...
import type { AsyncPlatformPilotReadinessPersistence } from "../infrastructure/asyncPlatformPilotReadinessPersistence.js";
...
export class PlatformPilotReadinessService {
  public constructor(private readonly repository:AsyncPlatformPilotReadinessPersistence,private readonly readiness:PilotReadinessService) {}
  public async workspace(publicId:unknown):Promise<PlatformPilotReadinessProjection>{
    ...
    const context=await this.repository.workspaceContext(publicId);if(!context)throw new Error("Workspace not found.");
    const companies=await Promise.all((await this.repository.readinessCompanies(context)).map(async company=>{
