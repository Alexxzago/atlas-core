...
test("EPIC045 PASS2 preserves outbound rowids on 0062 upgrade and restart", () => {
  ...
  try {
    ...
    assert.equal((db.prepare("SELECT id,name FROM schema_migrations ORDER BY id DESC LIMIT 1").get() as { id: number; name: string }).name, "0069_shared_rate_limit_windows");
