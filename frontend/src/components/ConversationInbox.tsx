...
export function ConversationInbox({ csrf, workspaceId, companyId, capabilities }: Props): React.JSX.Element {
  ...
  const send = async (): Promise<void> => {
    ...
    const submittedRevision = draftRevision.current; sendInFlight.current = true; setContent(""); setSending(true); setDetailError(false);
    try { const result = await atlasApi.sendConversationMessage(csrf, workspaceId, companyId, selected.conversationId, submitted, crypto.randomUUID()); setSelected((current) => current?.conversationId === selected.conversationId && !current.messages.some((message) => message.messageId === result.message.messageId) ? { ...current, messages: [...current.messages, { ...result.message, senderRole: "operator", deliveryCategory: "sent", delivery: { state: result.delivery.state, updatedAt: result.message.createdAt, safeErrorCategory: null } }] } : current); scrollToLatest(); void loadDetail(selected.conversationId); }
    catch (error) { if (!aborted(error)) { if (draftRevision.current === submittedRevision) setContent(submitted); if (selectedId.current === selected.conversationId) setDetailRefreshError(true); else setDetailError(true); } }
