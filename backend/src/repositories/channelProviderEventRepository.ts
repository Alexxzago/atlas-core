...
interface RequestRow { id:string;channel_provider_event_id:string;state:string;media_gate_state:"open"|"blocked_by_media"|"blocked_by_transcript";snapshot_json:string;lease_owner:string|null;lease_expires_at:string|null;outcome:string|null;created_at:string;updated_at:string; }
...
export class ChannelProviderEventRepository implements ChannelProviderEventRepositoryPort {
  ...
  public recomputeExecutionMediaGate(context: WorkspaceContext, companyId: number, connectionId: string, executionRequestId: ChannelExecutionRequestId, updatedAt: string): MediaGateRecomputeOutcome {
    ...
    try {
      ...
      if (current.media_gate_state === "blocked_by_transcript") { this.db.exec("COMMIT;"); return { kind: "unchanged", gate: "blocked_by_transcript" }; }
      if (media.length === 0) {
        this.db.exec("COMMIT;");
  ...
  public suppressBlockedAudioExecution(context: WorkspaceContext, companyId: number, connectionId: string, executionRequestId: ChannelExecutionRequestId, updatedAt: string): ChannelExecutionRequest | null {
    const changed = this.db.prepare("UPDATE channel_execution_requests SET state='completed',outcome='suppressed',lease_owner=NULL,lease_expires_at=NULL,updated_at=? WHERE id=? AND state='pending' AND media_gate_state='blocked_by_media' AND EXISTS(SELECT 1 FROM channel_provider_events e JOIN whatsapp_connections wc ON wc.id=e.transport_connection_id JOIN whatsapp_inbound_media m ON m.channel_provider_event_id=e.id WHERE e.id=channel_execution_requests.channel_provider_event_id AND wc.workspace_id=? AND wc.company_id=? AND wc.id=? AND m.provider_kind='audio')").run(updatedAt, executionRequestId, context.workspaceId, companyId, connectionId).changes === 1;
    if (!changed) return null;
    const row = this.db.prepare("SELECT * FROM channel_execution_requests WHERE id=?").get(executionRequestId) as RequestRow | undefined;
    return row ? request(row) : null;
  }
  public markBlockedAudioExecutionUnsupported(context: WorkspaceContext, companyId: number, connectionId: string, executionRequestId: ChannelExecutionRequestId, updatedAt: string): ChannelExecutionRequest | null {
