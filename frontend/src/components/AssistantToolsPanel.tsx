...
import { Badge, ErrorState, LoadingState } from "../design-system/primitives";
...
export function AssistantToolsPanel({ workspaceId, companyId, profileId, onNavigate }: Props): React.JSX.Element {
  ...
  return <section className="authenticated-section assistant-tools" aria-labelledby="assistant-tools-title">
    ...
    {tools.length === 0 ? <p>{t("tools.empty")}</p> : <ul className="assistant-tools__list" aria-label={t("tools.listLabel")}>{tools.map((tool) => {
      ...
      return <li key={tool.id} className="assistant-tool-row"><div><h3>{t(item?.name ?? "tools.generic.name")}</h3><p>{t(item?.purpose ?? "tools.generic.purpose")}</p>{!tool.enabled && <p className="assistant-tool-row__guidance">{t("tools.capabilityOff")}</p>}{!available && (tool.safeReason || tool.safeNextAction) && <div className="assistant-tool-row__guidance">{tool.safeReason && <p>{tool.safeReason}</p>}{tool.safeNextAction && <p>{tool.safeNextAction}</p>}</div>}</div><div className="assistant-tool-row__meta"><p><Badge tone={tool.enabled?"success":"neutral"}>{tool.enabled ? t("tools.enabled") : t("tools.notEnabled")}</Badge><Badge tone={available?"success":"warning"}>{t(`tools.${tool.availability}`)}</Badge></p><a href={capabilitiesPath} onClick={(event) => { event.preventDefault(); onNavigate?.(capabilitiesPath); }}>{t("tools.openCapabilities")}</a></div></li>;
    })}</ul>}
  </section>;
}
