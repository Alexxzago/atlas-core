...
test("EPIC050 pages filtered inboxes and keeps actor read positions isolated",async()=>{
  ...
  try {
    ...
    const first=await service.listInbox(context,company.id,"actor-a" as never,{limit:"1"});assert.equal(first.items.length,1);assert.equal(first.items[0]!.channel,"whatsapp");assert.equal(first.items[0]!.contactLabel,"+549 *** 5678");assert.equal((await service.detail(context,company.id,first.items[0]!.conversationId,"actor-a" as never)).contactLabel,first.items[0]!.contactLabel);assert.ok(first.nextCursor);
    const next=await service.listInbox(context,company.id,"actor-a" as never,{limit:"1",cursor:first.nextCursor!});assert.equal(next.items[0]!.channel,"web_chat");
    assert.equal((await service.listInbox(context,company.id,"actor-a" as never,{unreadOnly:"true"})).items.length,2);await service.markRead(context,company.id,first.items[0]!.conversationId,"actor-a" as never);assert.equal((await service.listInbox(context,company.id,"actor-a" as never,{unreadOnly:"true"})).items.length,1);assert.equal((await service.listInbox(context,company.id,"actor-b" as never,{unreadOnly:"true"})).items.length,2);
