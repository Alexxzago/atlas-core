...
function setup(state: "automated" | "human_required" | "human_controlled" = "human_controlled", controller = "operator-1") {
  ...
  const messages: Array<{ id: string; direction: string; senderParticipantId: string; idempotencyKey: string; content: string }> = [];
  ...
  const service = new OperatorConversationMessagingService(
    { validateOpen: () => ({ id: conversation }), addParticipant: (_c: unknown, _company: unknown, _conversation: unknown, value: { type: string; reference: string }) => { const participant = { id: conversationParticipantId(`cpt_${(participants.length + 1).toString(16).padStart(32, "0")}`), ...value }; participants.push(participant); return participant; }, addMessage: (_c: unknown, _company: unknown, _conversation: unknown, value: { senderParticipantId: string; idempotencyKey: string; content: string }) => { const message = { id: conversationMessageId(`cmsg_${(messages.length + 1).toString(16).padStart(32, "0")}`), direction: "outbound", ...value }; messages.push(message); return message; } } as never,
    { persistOperatorMessage: (_c: unknown, _company: unknown, _conversation: unknown, actorId: string, content: string, key: string) => { if (control.state !== "human_controlled" || control.controllingActorId !== actorId) return { kind: "forbidden" }; const existing = messages.find((message) => message.idempotencyKey === key); if (existing) return existing.content === content ? { kind: "replayed", message: existing, deliveryId: "odl_0123456789abcdef0123456789abcdef" } : { kind: "idempotency_mismatch" }; const operator = participants.find((value) => value.type === "human_operator" && value.reference === actorId) ?? { id: conversationParticipantId(`cpt_${(participants.length + 1).toString(16).padStart(32, "0")}`), type: "human_operator", reference: actorId }; if (!participants.includes(operator)) participants.push(operator); const created = { id: conversationMessageId(`cmsg_${(messages.length + 1).toString(16).padStart(32, "0")}`), direction: "outbound", senderParticipantId: operator.id, idempotencyKey: key, content }; messages.push(created); activity += 1; return { kind: "created", message: created, deliveryId: "odl_0123456789abcdef0123456789abcdef" }; } } as never,
    { ensureConversationControl: () => control, findConversationControl: () => control, updateConversationOperatorActivity: (_c: unknown, _company: unknown, _conversation: unknown, actorId: string, activityAt: string, updatedAt: string) => { if (control.state !== "human_controlled" || control.controllingActorId !== actorId) return null; activity += 1; control = reconstructConversationControl({ ...control, lastOperatorActivityAt: activityAt, updatedAt }); return control; } } as never,
