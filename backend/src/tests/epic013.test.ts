...
import type { OperationalExecutionBudgetPort } from "../assistant/application/operationalExecutionBudget.js";
...
const knowledgeVersion = { id: "kver_0123456789abcdef0123456789abcdef", companyId: 1, versionNumber: 1, compilerVersion: "company-knowledge-compiler-v1" as const, knowledge, snapshotDigest: "a".repeat(64), publishedByActorId: "system:test", publishedAt: "2026-01-01T00:00:00.000Z", sourceRevisionIds: [], publicationVersion: 1 };
...
const published = { load: () => knowledge, loadCurrentVersion: () => knowledgeVersion } as KnowledgeRepositoryPort & { loadCurrentVersion: () => typeof knowledgeVersion | null };
function runtime(execution: AssistantExecutionPort): OperationalAssistantRuntime { return new OperationalAssistantRuntime(execution, { create: (record) => record, complete: () => true }, { now: () => "2026-01-01T00:00:00.000Z" }); }
function createService(execution: AssistantExecutionPort, knowledgePort = published, companyPort = companies, profilePort = profiles, budget: OperationalExecutionBudgetPort = new InMemoryOperationalExecutionBudget()): OperationalAssistantExecutionService { return new OperationalAssistantExecutionService(companyPort, knowledgePort, profilePort, runtime(execution), budget, "test"); }

test("operational execution uses the immutable published request and normalizes provider fallback", async () => {
  const execution = new Execution(); const service = createService(execution);
...
test("operational execution budget limits concurrent trusted Workspace execution", async () => {
  ...
  const service = createService(new Execution(), published, companies, profiles, { acquire: () => null });
...
test("operational execution stops before the port for missing scope or publication and releases failed leases", async () => {
  ...
  await assert.rejects(() => createService(execution, published, { ...companies, findById: () => null }).execute(context, 1, { assistantProfileId: profile.id, message: "Question" }), OperationalAssistantExecutionNotFoundError);
  await assert.rejects(() => createService(execution, published, companies, { ...profiles, findById: () => null }).execute(context, 1, { assistantProfileId: profile.id, message: "Question" }), OperationalAssistantExecutionNotFoundError);
  await assert.rejects(() => createService(execution, { load: () => null, loadCurrentVersion: () => null }).execute(context, 1, { assistantProfileId: profile.id, message: "Question" }), OperationalAssistantKnowledgeUnavailableError);
  assert.equal(execution.calls, 0);
  const budget = new InMemoryOperationalExecutionBudget(); execution.error = new Error("unexpected");
  const service = createService(execution, published, companies, profiles, budget);
...
test("operational execution validates the exact DTO and rejects non-executable Profiles before Knowledge", async () => {
  ...
  await assert.rejects(() => createService(execution, published, companies, { ...profiles, findById: () => ({ ...profile, status: "draft" }) }).execute(context, 1, { assistantProfileId: profile.id, message: "Question" }), OperationalAssistantProfileNotExecutableError);
  assert.equal(execution.calls, 0);
});
...
test("operational execution keeps unpublished Knowledge and ineligible requests away from the port and budget", async () => {
  ...
  const publishedOnly = { load: () => { knowledgeLoads++; return publication; }, loadCurrentVersion: () => publication ? knowledgeVersion : null } as KnowledgeRepositoryPort & { loadCurrentVersion: () => typeof knowledgeVersion | null };
  const budget = { acquire: () => { budgetAcquires++; return { release: () => undefined }; } };
  const input = { assistantProfileId: profile.id, message: "Question" };
  const execute = (companyPort = companies, profilePort = profiles) => createService(execution, publishedOnly, companyPort, profilePort, budget).execute(context, 1, input);
...
test("operational execution does not invoke its port when the trusted Workspace budget is exhausted", async () => {
  const execution = new Execution();
  const service = createService(execution, published, companies, profiles, { acquire: () => null });
  await assert.rejects(() => service.execute(context, 1, { assistantProfileId: profile.id, message: "Question" }), OperationalAssistantExecutionRateLimitedError);
  assert.equal(execution.calls, 0);
});
