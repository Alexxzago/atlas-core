...
function AccountMenu({ ref, style, ...props }: AppShellProps & { readonly navigate: Navigate; readonly ref: React.Ref<HTMLDivElement>; readonly style: React.CSSProperties }): React.JSX.Element {
  ...
  return <div ref={ref} style={style} className="workspace-menu" role="dialog" aria-modal="false" aria-label={t("shell.workspaceMenu")}>
    <section><p>{t("shell.menu.workspace")}</p><strong>{props.workspace?.name ?? t("shell.noWorkspace")}</strong>{props.workspaces.length > 1 && <label><span>{t("shell.workspaceContext")}</span><select value={props.workspace?.id ?? ""} onChange={(event) => { if (event.target.value) props.onSelectWorkspace(event.target.value); }}>{props.workspaces.map((workspace) => <option key={workspace.id} value={workspace.id}>{workspace.name}</option>)}</select></label>}<button type="button" onClick={() => props.navigate("/billing")}>{t("billing.title")}</button><button type="button" onClick={() => props.navigate("/settings")}>{t("shell.workspaceSettings")}</button></section>
