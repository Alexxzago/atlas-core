...
test("EPIC044 upgrades 0059 data without changing delivery order or event cursors", () => {
  ...
  try {
    ...
    assert.deepEqual({ ...(reopened.prepare("SELECT id,name FROM schema_migrations ORDER BY id DESC LIMIT 1").get() as Record<string, unknown>) }, { id: 71, name: "0071_conversation_resume_operation" });
