...
export function ConversationInbox({ csrf, workspaceId, companyId, capabilities }: Props): React.JSX.Element {
  ...
  const listAbort = useRef<AbortController | null>(null), moreAbort=useRef<AbortController|null>(null), detailAbort = useRef<AbortController | null>(null), mutationAbort = useRef<AbortController | null>(null), feedAbort = useRef<AbortController | null>(null), feedTimer = useRef<number | null>(null), detailHeading = useRef<HTMLHeadingElement>(null), timeline = useRef<HTMLOListElement>(null), selectedId = useRef<string | null>(null), itemsRef=useRef<typeof items>([]), listRequestId = useRef(0), detailRequestId = useRef(0), operationIds = useRef(new Map<string, string>()), wasAtLatest = useRef(true), timelineIdentity = useRef<string | null>(null);
  ...
  const loadDetail = useCallback(async (id: string, focus = false, preserve = false): Promise<boolean> => {
    if (!workspaceId || !companyId) return false;
    if (focus && selectedId.current !== id) { timelineIdentity.current = null; wasAtLatest.current = true; setShowJumpToLatest(false); }
    ...
    setDetailLoading(true); if (!preserve) setDetailError(false); setDetailRefreshError(false);
    try { const detail = await atlasApi.getConversation(workspaceId, companyId, id, controller.signal); if (requestId === detailRequestId.current && !controller.signal.aborted) { setSelected(detail); if (focus) setMobileDetail(true); return true; } return false; }
    catch (cause) { if (requestId === detailRequestId.current && !aborted(cause)) { if (preserve && selectedId.current === id) setDetailRefreshError(true); else setDetailError(true); } return false; }
  ...
  const load = useCallback(async (preserve = false): Promise<boolean> => {
    ...
    try {
      ...
      if (retained) await loadDetail(retained, false, true); else setSelected(null);
      return true;
  ...
  const control = async (action: "take" | "release" | "resolve" | "resume"): Promise<boolean> => {
    if (!workspaceId || !companyId || !selected || controlsDisabled || ((action === "release" || action === "resolve") && !selected.controlledByCurrentActor) || (action === "resume" && selected.controlState !== "human_required")) return false;
    ...
    try {
      ...
      operationIds.current.delete(key); setSelected((current) => current?.conversationId === selected.conversationId ? { ...current, ...response.control } : current); await loadDetail(selected.conversationId); return true;
    } catch (error) { if (!aborted(error)) { setDetailError(true); if (error instanceof ApiError && error.status === 409) await load(); } return false; }
  ...
  const send = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    mutationAbort.current?.abort(); const controller = new AbortController(); mutationAbort.current = controller; setWorking(true); setDetailError(false);
    try { await atlasApi.sendConversationMessage(csrf, workspaceId, companyId, selected.conversationId, content.trim(), crypto.randomUUID()); setContent(""); scrollToLatest(); await loadDetail(selected.conversationId, false, true); }
    catch (error) { if (!aborted(error)) setDetailError(true); }
    finally { if (mutationAbort.current === controller) setWorking(false); }
  };
  const select = async (id: string): Promise<void> => { if (selected && selected.conversationId !== id && selected.controlledByCurrentActor && !await control("release")) return; await loadDetail(id, true); if (workspaceId && companyId) { try { await atlasApi.markConversationRead(csrf,workspaceId,companyId,id); setItems(current=>current.map(item=>item.conversationId===id?{...item,unreadCount:0}:item)); setSelected(current=>current?.conversationId===id?{...current,unreadCount:0}:current); } catch { /* Reading must not prevent inspecting the conversation. */ } } };
