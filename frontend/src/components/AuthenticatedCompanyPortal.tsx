...
function AuthenticatedCompanyPortalContent({ csrf, userId, email, isPlatformAdmin, onPassword, onLogout }: Props): React.JSX.Element {
  ...
  useEffect(()=>{if(route.name==="company-assistant-section"&&route.section!=="general"&&route.section!=="behavior")navigate(`/companies/${route.companyId}/assistant`,{replace:true});},[route,navigate]);
  ...
  const assistantSection=route.name==="company-assistant-section"&&(route.section==="general"||route.section==="behavior")?route.section:undefined;
