...
try{process.exitCode=await runBackupCommand(process.argv.slice(2),{buckets:{live:liveConfiguration.bucket,backup:backupConfiguration.bucket},run:async()=>new BackupOrchestrationService(media,new S3BackupObjectStorage(backupConfiguration)).create({writesQuiesced:true,sourceDatabase:process.env.TURSO_DATABASE_NAME?.trim()??"",migrationHead:(await database.query<{name:string}>("SELECT name FROM schema_migrations ORDER BY id DESC LIMIT 1"))[0]?.name??"",sourceBucket:liveConfiguration.bucket,backupBucket:backupConfiguration.bucket,applicationBuild:process.env.ATLAS_BUILD?.trim()??"unknown",retentionDays:Number(process.env.ATLAS_BACKUP_RETENTION_DAYS??"30")}),write:value=>console.log(value.trimEnd())});}finally{await database.close();}
