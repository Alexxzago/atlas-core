...
function AuthenticatedCompanyPortalContent({ csrf, email, onPassword, onLogout }: Props): React.JSX.Element {
  ...
  useEffect(() => {
    if (requestedCompanyId || !state.selectedWorkspace || state.companiesLoading || state.companyError || state.selectedCompanyId !== null || state.companies.length === 0) return;
    let current = true;
    const companyId = state.companies[0]!.id;
    void selectCompany(companyId).then((selected) => { if (current && selected) navigate(`/companies/${companyId}`, { replace: true }); });
    return () => { current = false; };
  ...
  return <AppShell route={route} workspace={state.selectedWorkspace} workspaces={state.workspaces} companies={state.companies} selectedCompany={selectedCompany} companiesLoading={state.companiesLoading} companyError={state.companyError} companyCreating={state.companyCreating} companyTransitioning={routeCompanyState === "loading"} email={email} onNavigate={navigate} onSelectWorkspace={(id) => { void selectWorkspace(id).then((selected) => { if (selected) navigate("/companies", { replace: true }); }); }} onSelectCompany={(id) => navigate(`/companies/${id}`)} onCreateCompany={createCompany} onRetryCompanies={refreshCompanies} onPassword={onPassword} onLogout={onLogout}>
