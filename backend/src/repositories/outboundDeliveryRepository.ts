...
export class OutboundDeliveryRepository implements OutboundDeliveryRepositoryPort {
  ...
  public authorizeLease(id: OutboundDeliveryId, owner: string, at: string): boolean {
    ...
    try {
      ...
      const authorized = current.expected_authority_generation === null || (current.control_state !== "human_controlled" && current.authority_generation === current.expected_authority_generation);
  ...
  public beginSend(id: OutboundDeliveryId, owner: string, at: string): boolean {
    ...
    try {
      ...
      const authorized = current && (current.expected_authority_generation === null || (current.control_state !== "human_controlled" && current.authority_generation === current.expected_authority_generation)) && (current.response_policy !== "deferred_voice" || (current.voice_ai_enabled === 1 && current.audio_response_mode === "voice_with_text_fallback"));
