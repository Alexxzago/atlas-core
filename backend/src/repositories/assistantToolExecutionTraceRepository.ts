...
export class AssistantToolExecutionTraceRepository implements ToolExecutionTraceRepositoryPort {
  ...
  public async complete(id: string, expectedState: "requested", value: { readonly auditOutput: unknown; readonly completedAt: string; readonly durationMilliseconds: number }): Promise<boolean> {
    const result=await this.database.execute(`UPDATE tool_execution_traces SET state='completed',audit_output_json=?,completed_at=?,duration_milliseconds=? WHERE id=? AND state=?`,[json(value.auditOutput),value.completedAt,value.durationMilliseconds,id,expectedState]); return Number(result.rowsAffected)===1;
  }
