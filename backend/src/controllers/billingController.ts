...
export function createBillingControllers(service:BillingApplicationService,limits?:RateLimitService):Record<"summary"|"offers"|"managementActions"|"entitlements"|"catalog"|"payerIdentityOptions"|"setPayerIdentity"|"clearPayerIdentity"|"checkout"|"checkoutOffer"|"portal"|"cancel"|"reactivate",(context:BillingRequestContext)=>RequestHandler> { const limit=(context:BillingRequestContext):void=>{limits?.enforce(abuseScope("workspace",context.workspaceId,"actor",context.callerIdentityId),"actor",billingActorLimit);limits?.enforce(abuseScope("workspace",context.workspaceId),"company",billingWorkspaceLimit);}; return {
  summary:context=>safe((_req,res)=>{ const result=service.customerSummary(context.workspaceId); if (!result) { res.status(404).json({ error:"Resource not found." }); return; } res.json(result); }),
  offers:context=>safe((_req,res)=>{const result=service.offersForWorkspace(context.workspaceId);if(!result){res.status(404).json({error:"Resource not found."});return;}res.json(result);}),
  managementActions:context=>safe((_req,res)=>{const result=service.managementActionsFor(context.workspaceId);if(!result){res.status(404).json({error:"Resource not found."});return;}res.json(result);}),
  ...
  checkoutOffer:context=>safe(async(req,res)=>{const body=exact(req.body,["offerId"]);limit(context);respond(res,await service.checkoutOffer(context.workspaceId,providerCommercialOfferId(body.offerId),idempotencyKey(req.headers["idempotency-key"])));}),
