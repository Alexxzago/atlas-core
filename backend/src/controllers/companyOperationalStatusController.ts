...
export function createGetCompanyOperationalStatusController(service: CompanyOperationalStatusService, context: WorkspaceContext): RequestHandler { return (request, response): void => { try { response.json(service.get(context, companyId(request.params.companyId))); } catch (error: unknown) { respond(response, error); } }; }
