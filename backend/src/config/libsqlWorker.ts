...
if (!workerPort) throw new Error("libSQL worker requires a parent port.");
...
workerPort.once("message", ({ port }: { readonly port: MessagePort }) => {
port.on("message", async (request: Request): Promise<void> => {
  try {
    ...
    if (request.action === "close") { if (transaction) await transaction.rollback(); client.close(); port.postMessage({ id: request.id, value: {} }); port.close(); workerPort.close(); return; }
