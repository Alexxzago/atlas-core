...
test("saving a draft reports saved configuration and clears the notice when opening review or edit", () => {
  const before = { ...populated(), formMode: "create" as const, activeMutationRequest: mutation(populated(), "create") };
  const saved = authenticatedPortalReducer(before, { type: "profileCreated", request: before.activeMutationRequest!, profile: profileA });
  assert.equal(saved.profiles[0]?.status, "draft");
  assert.deepEqual(saved.notice, { type: "success", key: "profiles.createSuccess" });
  assert.equal(authenticatedPortalReducer(saved, { type: "profileSelected", profileId: profileA.id }).notice, null);
  assert.equal(authenticatedPortalReducer(saved, { type: "formOpened", mode: "edit" }).notice, null);
});
...
test("tenant non-disclosure 404 recovers only when the exact generic response identifies it",async()=>{const originalFetch=globalThis.fetch;let calls=0,recoveries=0;globalThis.fetch=async()=>{calls+=1;return calls===1?new Response(JSON.stringify({error:"Resource not found."}),{status:404,headers:{"content-type":"application/json"}}):new Response(JSON.stringify({data:[]}),{status:200,headers:{"content-type":"application/json"}});};setAuthenticationRecovery(async method=>{recoveries+=1;assert.equal(method,"GET");return true;});try{assert.deepEqual(await atlasApi.listWorkspaceCompanies("wsp_one"),[]);assert.equal(calls,2);assert.equal(recoveries,1);}finally{setAuthenticationRecovery(null);globalThis.fetch=originalFetch;}});

test("a genuine tenant 404 remains a 404 without authentication recovery",async()=>{const originalFetch=globalThis.fetch;let recoveries=0;globalThis.fetch=async()=>new Response(JSON.stringify({error:"Company was not found."}),{status:404,headers:{"content-type":"application/json"}});setAuthenticationRecovery(async()=>{recoveries+=1;return true;});try{await assert.rejects(()=>atlasApi.listWorkspaceCompanies("wsp_one"),(error:unknown)=>error instanceof ApiError&&error.status===404);assert.equal(recoveries,0);}finally{setAuthenticationRecovery(null);globalThis.fetch=originalFetch;}});

test("tenant non-disclosure never replays a mutation after recovery",async()=>{const originalFetch=globalThis.fetch;let calls=0,recoveries=0;globalThis.fetch=async()=>{calls+=1;return new Response(JSON.stringify({error:"Resource not found."}),{status:404,headers:{"content-type":"application/json"}});};setAuthenticationRecovery(async method=>{recoveries+=1;assert.equal(method,"POST");return true;});try{await assert.rejects(()=>atlasApi.createOnboardingCompany("csrf","wsp_one",{name:"Company"}),(error:unknown)=>error instanceof ApiError&&error.status===404);assert.equal(calls,1);assert.equal(recoveries,1);}finally{setAuthenticationRecovery(null);globalThis.fetch=originalFetch;}});
...
test("Workspace and Company list adapters normalize their production response contracts", async () => {
  ...
  try {
    ...
    assert.deepEqual(await atlasApi.listWorkspaceCompanies(workspaceA.id), [{ id: 7, name: "Core Company", website: null, phone: "", email: "", status: "ready", lifecycle: "operational", createdAt: "2026-08-01T00:00:00.000Z" }]);
