...
interface Row { id:string;company_id:number;assistant_profile_id:string;profile_snapshot_json:string;knowledge_version_id:string;execution_snapshot_json:string|null;provider:string;purpose:AssistantRuntimePurpose;state:AssistantExecutionRecordState;fallback_used:number;result:string|null;input_tokens:number|null;output_tokens:number|null;error_code:string|null;started_at:string;completed_at:string|null;duration_milliseconds:number|null;version_number:number;snapshot_digest:string; }
  ...
  public create(value: AssistantExecutionRecord): AssistantExecutionRecord {
    const result = this.db.prepare(`
      INSERT INTO assistant_execution_records (
        id, company_id, assistant_profile_id, profile_snapshot_json, knowledge_version_id, execution_snapshot_json, provider, purpose,
      ...
      WHERE c.id = ?
    `).run(value.id, JSON.stringify(value.profileSnapshot), JSON.stringify(value.executionSnapshot), value.provider, value.purpose, value.state, value.fallbackUsed ? 1 : 0,
