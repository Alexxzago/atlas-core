...
test("bootstrap owner keeps the default Workspace through login, proxy-origin selection, companies, and Workspace creation", async () => {
  ...
  app.use("/workspaces", createAuthorizedCompaniesRouter({ authentication, users: new UserRepository(database), authorization: new AuthorizationService(memberships, workspaces), resolver: new WorkspaceResolver(workspaces), commercial: new CommercialControlsRepository(database), controllers: { list: (context) => createListCompaniesController(companies, context), create: (context) => createCompanyController(companies, context), get: (context) => createGetCompanyController(companies, context), update: (context) => createUpdateCompanyController(companies, context), delete: (context) => createDeleteCompanyController(companies, context), onboard: () => unavailable }, assistantControllers: { list: () => unavailable, create: () => unavailable, get: () => unavailable, update: () => unavailable, transition: () => unavailable, preview: () => unavailable } }));
  const server = app.listen(0, "127.0.0.1");
  await new Promise<void>((resolve, reject) => { server.once("listening", resolve); server.once("error", reject); });
  ...
  try {
    ...
    assert.equal((await fetch(`${base}/workspaces/${defaultWorkspace.id}/companies`, { headers })).status, 200);
    assert.equal((await fetch(`${base}/workspaces/${defaultWorkspace.id}/companies`, { method: "POST", headers, body: JSON.stringify({ name: "Blocked Company", website: "https://blocked-company.test" }) })).status, 409);
    const reactivation = controls.workspaceByPublicId(defaultWorkspace.id)!;
    assert.equal(controls.setWorkspaceStatus(ownerId, reactivation.workspaceId, "active", reactivation.version, clock.now())?.status, "active");
    const ownerControls = controls.user(ownerId)!;
    assert.equal(controls.updateUser(ownerId, ownerId, 2, ownerControls.version, clock.now())?.maxOwnedWorkspaces, 2);
