...
export class KnowledgeIndexingService {
  ...
  public async indexCompletedRevision(context:WorkspaceContext,companyId:number,input:{sourceId:string;sourceRevisionId:string;contentDigest:string;normalizedText:string;completedAt:string}):Promise<void>{await this.repository.replaceRevision(context,companyId,{...input,createdAt:input.completedAt,chunks:chunkUtf8Deterministically(input.normalizedText)});}
  public async publicationReady(context:WorkspaceContext,companyId:number,revisionIds:readonly string[]):Promise<boolean>{return await this.repository.readyForRevisions(context,companyId,revisionIds);}
}
export class LexicalKnowledgeRetrievalService implements KnowledgeRetrievalPort {
  public constructor(private readonly repository:KnowledgeRetrievalRepositoryPort){}
  public async retrieve(context:WorkspaceContext,companyId:number,revisionIds:readonly string[],query:string,limit=6):Promise<readonly KnowledgeChunkV2[]>{const terms=lexicalTerms(query);if(!terms.length||!Number.isSafeInteger(limit)||limit<1)return[];const seen=new Set<string>();return Object.freeze((await this.repository.findReadyChunks(context,companyId,revisionIds)).map(chunk=>({chunk,score:terms.reduce((total,term)=>total+occurrences(chunk.normalizedText,term),0)})).filter(match=>match.score>0).sort((a,b)=>b.score-a.score||a.chunk.sourceRevisionId.localeCompare(b.chunk.sourceRevisionId)||a.chunk.ordinal-b.chunk.ordinal).filter(match=>{if(seen.has(match.chunk.id))return false;seen.add(match.chunk.id);return true;}).slice(0,limit).map(match=>match.chunk));}
  ...
  public async context(context:WorkspaceContext,companyId:number,revisionIds:readonly string[],query:string):Promise<RetrievalContext>{return this.assemble(await this.retrieve(context,companyId,revisionIds,query));}
}
