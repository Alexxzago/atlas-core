...
test("EPIC046 migrates fresh and staged file-backed databases, backfills unmanaged accounts, and restarts", () => {
  ...
  try {
    ...
    assert.equal(head.id, 73); assert.equal(head.name, "0073_scheduling_configuration_controls");
...
test("EPIC046 PASS4F5 HTTP summary and entitlements expose only safe unmanaged projections", async () => withBillingHttp(async fixture => {
  ...
  assert.equal(summary.status,200); assert.equal(summary.headers.get("cache-control"),"no-store, private"); assert.equal(summary.headers.get("pragma"),"no-cache"); assert.deepEqual(await summary.json(),{rolloutMode:"unmanaged",subscription:{state:"unmanaged",plan:null}});
  assert.equal(entitlements.status,200); assert.deepEqual(Object.keys(await entitlements.json()).sort(),["effectiveAt","expiresAt","maxActiveChannels","maxAssistantProfiles","maxCompanies","mutationEligible","state"]);
}));
...
test("EPIC046 PASS4F5 HTTP projects paused, administrative, minimum, and zero entitlement states without activation", async () => withBillingHttp(async fixture => {
  ...
  assert.deepEqual(await (await fixture.get("/summary")).json(),{rolloutMode:"unmanaged",subscription:{state:"reconciliation_required",plan:null}});
...
test("EPIC046 PASS4F5 HTTP checkout permits Stripe without a payer and requires the selected Mercado Pago payer", async () => withBillingHttp({providers:["stripe","mercadopago"]},async fixture => {
  ...
  assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:stripe.id},{"idempotency-key":"stripe"})).status,200);
  assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:mercadoPago.id},{"idempotency-key":"mp-missing"})).status,409);
  ...
  assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:mercadoPago.id},{"idempotency-key":"mp-selected"})).status,200); assert.equal(fixture.mercadoPago!.calls.at(-1)?.payerEmail,"payer@example.test");
}));
...
test("EPIC046 PASS4F7B selects a second own identity for Mercado Pago checkout", async () => withBillingHttp({providers:["stripe","mercadopago"]},async fixture => {
  ...
  const checkout=await fixture.post("/checkout-sessions",{catalogEntryId:mercadoPago.id},{"idempotency-key":"mp-owned-second"}); assert.equal(checkout.status,200); assert.equal(fixture.mercadoPago!.calls.length,1); assert.equal(fixture.mercadoPago!.calls[0]!.payerEmail,"second@example.test");
}));
...
test("EPIC046 PASS4F5 HTTP validates idempotency and exact mutation bodies before dispatch", async () => withBillingHttp(async fixture => {
  ...
  assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:entry.id,successTarget:"https://evil.test"},{"idempotency-key":"valid"})).status,400);
  assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:entry.id})).status,400);
  assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:entry.id},{"idempotency-key":"bad key"})).status,400);
...
test("EPIC046 PASS4F5 HTTP replays exact checkout and conflicts divergent durable reuse", async () => withBillingHttp(async fixture => {
  ...
  assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:first.id},{"idempotency-key":"same-key"})).status,200); assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:first.id},{"idempotency-key":"same-key"})).status,200); assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:second.id},{"idempotency-key":"same-key"})).status,409); assert.equal(fixture.stripe.calls.length,1);
}));
...
test("EPIC046 PASS4F5 HTTP maps provider outcomes without revealing provider internals", async () => {
  for (const [result,status] of [[{kind:"failed",code:"unavailable"},503],[{kind:"uncertain"},202]] as const) await withBillingHttp({result},async fixture => { const entry=fixture.catalog({planKey:`http-${result.kind}`,providerKind:"stripe",providerPriceId:`price-${result.kind}`}), response=await fixture.post("/checkout-sessions",{catalogEntryId:entry.id},{"idempotency-key":result.kind}); assert.equal(response.status,status); assert.deepEqual(await response.json(),{status:result.kind}); });
});
...
async function withBillingHttp(options:BillingHttpOptions|((fixture:BillingHttpFixture)=>Promise<void>), operation?:((fixture:BillingHttpFixture)=>Promise<void>)):Promise<void> {
  ...
  const billingRouter=createBillingRouter({authentication:{cookieName:()=>"atlas",current:(raw:string)=>raw==="manager"||raw==="reader"?{userId:raw,authenticationIdentityId:`${raw}-identity`}:null,validateCsrf:(_raw:string,csrf:string)=>csrf==="csrf"}as never,users:{findById:(id:string)=>({id,status:"active"})}as never,authorization:{authorize:(user:{id:string},workspace:string,permission:string)=>{if(user.id!=="manager"||workspace!=="wsp_default"||permission!=="workspace:manage")throw new Error("denied");return{workspaceId,workspacePublicId:workspace,userId:user.id,membershipId:"mem",role:"owner",capabilities:new Set([permission]),permission};}}as never,resolver:{resolve:()=>({workspaceId,workspaceKey:"default"})}as never,originPolicy:{allows:()=>true}as never,controllers:createBillingControllers(service)}), empty=Router(), webhook=config.webhook?createBillingWebhookRouter({stripe:createBillingWebhookController(new BillingWebhookService({stripe:"stripe_webhook_secret",mercadopago:"mp_webhook_secret"},new BillingWebhookRepository(db),()=>at),"stripe"),mercadoPago:createBillingWebhookController(new BillingWebhookService({stripe:"stripe_webhook_secret",mercadopago:"mp_webhook_secret"},new BillingWebhookRepository(db),()=>at),"mercadopago")}):undefined, app=createApp({authorizedCompaniesRouter:empty,billingRouter,...(webhook?{billingWebhookRouter:webhook}:{}),chatRouter:empty,companiesRouter:empty,identityRouter:empty,knowledgeRouter:empty,publicWebChatRouter:empty,scrapeRouter:empty,workspacesRouter:empty}), server=app.listen(0,"127.0.0.1");
...
test("EPIC046 Recovery H HTTP checkout replays a recovered result without a second provider dispatch", async () => withBillingHttp({result:{kind:"uncertain"}},async fixture => {
  const entry=fixture.catalog({planKey:"http-recovery-replay",providerKind:"stripe",providerPriceId:"price_http_recovery"}); assert.equal((await fixture.post("/checkout-sessions",{catalogEntryId:entry.id},{"idempotency-key":"recover-http"})).status,202); const operations=new BillingOperationRepository(fixture.db),operationId=(fixture.db.prepare("SELECT operation_id FROM billing_operations WHERE billing_account_id=? AND operation_kind='checkout_session_create'").get(fixture.account.id) as {operation_id:string}).operation_id,operation=operations.find(fixture.account.id,"checkout_session_create",operationId)!,claim=operations.claimRecovery("http",at,"2026-09-01T00:00:00.000Z","2026-09-01T00:01:00.000Z")!; assert.ok(operations.succeedRecoveredCheckout(claim,"cs_http_recovered",JSON.stringify({providerObjectId:"cs_http_recovered",redirectUrl:"https://atlas.test/recovered"}),at)); const replay=await fixture.post("/checkout-sessions",{catalogEntryId:entry.id},{"idempotency-key":"recover-http"}); assert.equal(replay.status,200); assert.deepEqual(await replay.json(),{status:"succeeded",redirectUrl:"https://atlas.test/recovered"}); assert.equal(fixture.stripe.calls.length,1); assert.equal(operation.status,"uncertain");
}));
