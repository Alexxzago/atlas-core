...
function originAllowed(request:Parameters<RequestHandler>[0],policy:RequestOriginPolicy):boolean{const authority=authorityResolver.resolve({protocol:request.protocol,host:request.headers.host,forwardedProtocol:request.headers["x-forwarded-proto"]});return !!authority&&policy.allows({origin:request.headers.origin,fetchSite:typeof request.headers["sec-fetch-site"]==="string"?request.headers["sec-fetch-site"]:undefined,effectiveProtocol:authority.protocol,effectiveAuthority:authority.authority});}
