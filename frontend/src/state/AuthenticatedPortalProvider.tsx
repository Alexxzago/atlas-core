...
export interface AuthenticatedPortalContextValue {
  ...
  reloadProfiles: () => void;
  submitProfile: (input: CreateAssistantProfileInput | UpdateAssistantProfileInput) => Promise<AssistantProfile | null>;
...
export function AuthenticatedPortalProvider({ csrf, children }: Props): React.JSX.Element {
  ...
  const submitProfile = async (input: CreateAssistantProfileInput | UpdateAssistantProfileInput): Promise<AssistantProfile | null> => { const current = stateRef.current, workspace = current.selectedWorkspace, companyId = current.selectedCompanyId; if (!workspace || !companyId) return null; mutationAbort.current?.abort(); const controller = new AbortController(); mutationAbort.current = controller; const operation: ProfileMutationOperation = current.formMode === "create" ? "create" : "update"; const request = nextMutation(operation, current.profileGeneration, workspace.id, companyId, operation === "update" ? current.selectedProfileId ?? undefined : undefined); dispatch({ type: "submissionStarted", request }); try { const profile = current.formMode === "create" ? await atlasApi.createAssistantProfile(csrf, workspace.id, companyId, input as CreateAssistantProfileInput, controller.signal) : current.selectedProfileId ? await atlasApi.updateAssistantProfile(csrf, workspace.id, companyId, current.selectedProfileId, input, controller.signal) : null; if (!profile) return null; dispatch({ type: operation === "create" ? "profileCreated" : "profileUpdated", request, profile }); return profile; } catch (error: unknown) { if (aborted(error)) { dispatch({ type: "requestAborted" }); return null; } if (error instanceof ApiError && error.status === 404) dispatch(operation === "create" ? { type: "profileCreateNotFound", request } : { type: "profileMutationNotFound", request }); else if (error instanceof ApiError && error.status === 409) dispatch({ type: "submissionFailed", request, noticeKey: operation === "create" ? "profiles.nameConflict" : "profiles.updateConflict" }); else if (error instanceof ApiError && error.status === 400) dispatch({ type: "submissionFailed", request, noticeKey: "profiles.validationError" }); else dispatch({ type: "submissionFailed", request, noticeKey: operation === "create" ? "profiles.createError" : "profiles.updateError" }); return null; } };
