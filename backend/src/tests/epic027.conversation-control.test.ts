...
function setup() {
  ...
  const controls = {
    applyConversationControlOperation: (_context: unknown, _companyId: unknown, _id: unknown, command: { operation: "takeover" | "release" | "resolve"; actorId: string; expectedVersion: number }) => {
      ...
      if (command.operation !== "takeover" && (control.state !== "human_controlled" || control.controllingActorId !== command.actorId)) return { kind: "rejected", outcome: "not_controller", control };
      const authority = applyConversationAuthorityTransition(control, { kind: command.operation, actorId: command.actorId as never });
      control = reconstructConversationControl(command.operation === "takeover" ? { ...control, ...authority, lastControllingActorId: command.actorId as never, takenAt: control.takenAt ?? at, releasedAt: null, attentionReason: "operator_follow_up", resolvedAt: null, resolvedBy: null, updatedAt: authority.version === control.version ? control.updatedAt : at } : command.operation === "release" ? { ...control, ...authority, releasedAt: at, attentionReason: "operator_follow_up", updatedAt: at } : { ...control, ...authority, releasedAt: at, attentionReason: null, resolvedAt: at, resolvedBy: command.actorId as never, updatedAt: at });
