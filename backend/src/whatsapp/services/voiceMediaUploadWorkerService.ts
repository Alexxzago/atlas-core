...
export class VoiceMediaUploadWorkerService {
  ...
  private async settle(context: WorkspaceContext, companyId: number, upload: WhatsAppOutboundMediaUpload, result: Awaited<ReturnType<WhatsAppOutboundMediaUploadPort["upload"]>>): Promise<VoiceMediaUploadWorkerOutcome> { if(result.kind!=="uploaded")operationalLogger.warn("voice_media_upload_failed",{subsystem:"voice",operation:"meta_upload",outcome:result.kind,workspaceId:context.workspaceId,companyId,safeErrorCategory:result.safeFailureCategory}); const saved = await this.voices.finalizeUpload(context, companyId, upload.id, this.options.owner, result, this.clock.now()); return { kind: saved?.state === "uploaded" ? "uploaded" : saved?.state === "expired" ? "retryable" : saved?.safeErrorCategory === "suppressed" ? "suppressed" : saved?.state === "failed" ? "failed" : "lease_lost", uploadId: upload.id }; }
}
