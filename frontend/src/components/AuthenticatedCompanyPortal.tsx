...
function AuthenticatedCompanyPortalContent({ csrf, userId, email, isPlatformAdmin, onPassword, onLogout }: Props): React.JSX.Element {
  ...
  const assistantSection=route.name==="company-assistant-section"&&(route.section==="general"||route.section==="behavior"||route.section==="capabilities"||route.section==="tools"||route.section==="status"||route.section==="test")?route.section:undefined;
  ...
  const routeContent = (): React.JSX.Element => {
    ...
    if (route.name === "company-assistant" || route.name === "company-assistant-section") return <div key={assistantContextKey}>{testPanel ?? assistantPanel}</div>;
