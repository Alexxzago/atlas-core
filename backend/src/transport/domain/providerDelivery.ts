...
export type ChannelExecutionRequestId = string & { readonly __brand: "ChannelExecutionRequestId" };
export type ProviderEventProcessingState = "claimed" | "processing" | "completed" | "failed";
export type OutboundDeliveryState = "pending" | "leased" | "accepted" | "delivered" | "read" | "retryable" | "permanent_failure" | "uncertain" | "blocked_by_synthesis" | "suppressed";
export type ChannelExecutionRequestState = "pending" | "leased" | "completed" | "failed" | "unsupported";
export type ChannelExecutionMediaGateState = "open" | "blocked_by_media" | "blocked_by_transcript";
export type MediaGateRecomputeOutcome = { readonly kind: "updated" | "unchanged"; readonly gate: ChannelExecutionMediaGateState } | { readonly kind: "not_found" | "conflict" };
...
export interface OutboundDelivery {
  ...
  readonly createdAt: string;
  readonly updatedAt: string;
  readonly payloadKind?: "text" | "deferred_voice" | "audio";
  readonly responsePolicy?: "standard" | "deferred_voice";
...
export interface ChannelExecutionRequest {
  readonly id: ChannelExecutionRequestId;
  readonly channelProviderEventId: ChannelProviderEventId;
  readonly state: ChannelExecutionRequestState;
  readonly mediaGateState?: ChannelExecutionMediaGateState;
...
export function channelExecutionRequestId(valueToValidate: string): ChannelExecutionRequestId { return opaque<ChannelExecutionRequestId>(valueToValidate, "cex"); }
...
export function providerExternalMessageId(valueToValidate: string): string { return value(valueToValidate, "External Message ID"); }
...
export function outboundDeliveryState(valueToValidate: string): OutboundDeliveryState { if (valueToValidate !== "pending" && valueToValidate !== "leased" && valueToValidate !== "accepted" && valueToValidate !== "delivered" && valueToValidate !== "read" && valueToValidate !== "retryable" && valueToValidate !== "permanent_failure" && valueToValidate !== "uncertain" && valueToValidate !== "blocked_by_synthesis" && valueToValidate !== "suppressed") throw new ProviderDeliveryDomainError("Outbound Delivery state is invalid."); return valueToValidate; }
export function channelExecutionRequestState(valueToValidate: string): ChannelExecutionRequestState { if (valueToValidate !== "pending" && valueToValidate !== "leased" && valueToValidate !== "completed" && valueToValidate !== "failed" && valueToValidate !== "unsupported") throw new ProviderDeliveryDomainError("Execution request state is invalid."); return valueToValidate; }
...
export function reconstructProviderMessageRecord(record: ProviderMessageRecord): ProviderMessageRecord { return Object.freeze({ ...record, id: providerMessageRecordId(record.id), communicationChannel: communicationChannel(record.communicationChannel), transportProvider: transportProvider(record.transportProvider), direction: conversationMessageDirection(record.direction), transportConnectionId: value(record.transportConnectionId, "Transport Connection ID"), conversationMessageId: conversationMessageId(record.conversationMessageId), externalMessageId: record.externalMessageId === null ? null : providerExternalMessageId(record.externalMessageId), createdAt: timestamp(record.createdAt), updatedAt: timestamp(record.updatedAt) }); }
export function reconstructOutboundDelivery(record: OutboundDelivery): OutboundDelivery { const owner = record.leaseOwner === null ? null : value(record.leaseOwner, "Lease Owner", 128), expiry = record.leaseExpiresAt === null ? null : timestamp(record.leaseExpiresAt), payloadKind=record.payloadKind??"text", responsePolicy=record.responsePolicy??"standard", mediaAssetId=record.mediaAssetId??null, expectedAuthorityGeneration=record.expectedAuthorityGeneration??null, sendStartedAt=record.sendStartedAt??null; if ((owner === null) !== (expiry === null) || !Number.isSafeInteger(record.attemptCount) || record.attemptCount < 0 || (payloadKind!=="text"&&payloadKind!=="deferred_voice"&&payloadKind!=="audio") || (responsePolicy!=="standard"&&responsePolicy!=="deferred_voice")) throw new ProviderDeliveryDomainError("Outbound Delivery state is invalid."); return Object.freeze({ ...record, id: outboundDeliveryId(record.id), providerMessageRecordId: providerMessageRecordId(record.providerMessageRecordId), transportConnectionId: value(record.transportConnectionId, "Transport Connection ID"), state: outboundDeliveryState(record.state), nextAttemptAt: timestamp(record.nextAttemptAt), leaseOwner: owner, leaseExpiresAt: expiry, safeErrorCategory: record.safeErrorCategory === null ? null : transportProvider(record.safeErrorCategory), payloadKind, responsePolicy, mediaAssetId, expectedAuthorityGeneration, sendStartedAt: sendStartedAt === null ? null : timestamp(sendStartedAt), createdAt: timestamp(record.createdAt), updatedAt: timestamp(record.updatedAt) }); }
export function reconstructChannelExecutionRequest(record: ChannelExecutionRequest): ChannelExecutionRequest { const owner = record.leaseOwner === null ? null : value(record.leaseOwner, "Lease Owner", 128), expiry = record.leaseExpiresAt === null ? null : timestamp(record.leaseExpiresAt), mediaGateState = record.mediaGateState ?? "open"; if ((owner === null) !== (expiry === null) || !record.snapshot || typeof record.snapshot !== "object" || Array.isArray(record.snapshot) || (mediaGateState !== "open" && mediaGateState !== "blocked_by_media" && mediaGateState !== "blocked_by_transcript")) throw new ProviderDeliveryDomainError("Execution request is invalid."); return Object.freeze({ ...record, id: channelExecutionRequestId(record.id), channelProviderEventId: channelProviderEventId(record.channelProviderEventId), state: channelExecutionRequestState(record.state), mediaGateState, snapshot: Object.freeze({ ...record.snapshot }), leaseOwner: owner, leaseExpiresAt: expiry, outcome: record.outcome === null ? null : value(record.outcome, "Execution outcome", 64), createdAt: timestamp(record.createdAt), updatedAt: timestamp(record.updatedAt) }); }
