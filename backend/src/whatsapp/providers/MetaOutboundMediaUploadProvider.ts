...
export class MetaOutboundMediaUploadProvider implements WhatsAppOutboundMediaUploadPort {
  public constructor(private readonly connections: WhatsAppConnectionRepositoryPort|AsyncWhatsAppConnectionRepositoryPort, private readonly credentials: WhatsAppCredentialResolverPort|AsyncWhatsAppCredentialResolverPort, private readonly graphVersion: string, private readonly fetcher: typeof fetch = fetch) {}
  public async upload(context: WorkspaceContext, companyId: number, connectionId: string, input: WhatsAppOutboundMediaUploadInput): Promise<WhatsAppOutboundMediaUploadResult> {
    ...
    try { token = await this.credentials.resolve(context, companyId, connection.id); } catch { return { kind: "failed", safeFailureCategory: "unauthorized" }; }
