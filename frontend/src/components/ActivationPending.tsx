...
export function ActivationPending(): React.JSX.Element {
  ...
  return <Surface className="guided-registration" tone="raised"><Stack gap="4"><h1 tabIndex={-1}>{t("activationPending.title")}</h1><p role="status" aria-live="polite">{readiness ? readiness.status === "ready" ? "Ready" : `Blocked: ${readiness.blockers.join(", ")}` : t("activationPending.description")}</p></Stack></Surface>;
}
