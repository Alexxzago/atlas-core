...
export interface PublicWebChatHistoryMessage { readonly direction: "inbound" | "outbound"; readonly content: string; readonly createdAt: string; }
...
export const publicWebChatApi = {
  ...
  sendMessage: (connectionPublicId: string, message: string, idempotencyKey: string): Promise<{ message: string }> => request(path(connectionPublicId, "messages"), { method: "POST", headers: { "content-type": "application/json", "idempotency-key": idempotencyKey }, body: JSON.stringify({ message }) }),
