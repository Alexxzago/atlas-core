...
type SmtpDeliveryPurpose = "email_verification" | "password_reset" | "credential_enrollment" | "invitation";
interface SmtpFailureLogEntry { readonly event: "smtp_delivery_failed"; readonly purpose: SmtpDeliveryPurpose; readonly outcome: Exclude<VerificationDeliveryOutcome, "accepted">; readonly errorCode: string | null; readonly responseCode: number | null; readonly command: string | null; readonly host: string; readonly port: number; readonly secure: boolean; readonly timestamp: string; }
...
function purpose(request: EmailVerificationDeliveryRequest | CredentialEnrollmentDeliveryRequest | PasswordResetDeliveryRequest | InvitationDeliveryRequest): SmtpDeliveryPurpose { return "verificationUrl" in request ? "email_verification" : "enrollmentUrl" in request ? "credential_enrollment" : "resetUrl" in request ? "password_reset" : "invitation"; }
...
export class SmtpEmailDelivery implements EmailVerificationDeliveryPort, CredentialEnrollmentDeliveryPort, PasswordResetDeliveryPort, InvitationDeliveryPort {
  ...
  public async deliver(request: EmailVerificationDeliveryRequest | CredentialEnrollmentDeliveryRequest | PasswordResetDeliveryRequest | InvitationDeliveryRequest): Promise<VerificationDeliveryOutcome> {
    ...
    try {
      ...
      this.logFailure({ event: "smtp_delivery_failed", purpose: purpose(request), outcome, errorCode: safeCode(details.code), responseCode: safeResponseCode(details.responseCode), command: safeCommand(details.command), host: this.configuration.host, port: this.configuration.port, secure: this.configuration.secure, timestamp: new Date().toISOString() });
      return outcome;
    }
  }
