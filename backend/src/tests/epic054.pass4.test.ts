...
class Attempts implements ActivationVerificationAttemptRepositoryPort {
  ...
  public async create(_context:typeof context,value:ActivationVerificationAttempt){this.values.push(value);return value;}
  public async findLatest(_context:typeof context,companyId:number,connectionId:string){return this.values.filter(value=>value.companyId===companyId&&value.webChatConnectionId===connectionId).at(-1)??null;}
  public async hasSucceeded(_context:typeof context,companyId:number,connectionId:string){return this.values.some(value=>value.companyId===companyId&&value.webChatConnectionId===connectionId&&value.status==="succeeded");}
  public async findClaimableByTokenDigest(digest:string){return this.values.find(value=>value.tokenDigest===digest&&value.status==="pending"&&value.claimedAt===null)??null;}
  public async claim(id:string,webChatSessionId:string,conversationId:string,at:string){const index=this.values.findIndex(value=>value.id===id&&value.status==="pending"&&value.claimedAt===null);if(index<0)return null;const value={...this.values[index]!,claimedAt:at,webChatSessionId,conversationId};this.values[index]=value;return value;}
  public async succeedForTurn(webChatSessionId:string,conversationId:string,inboundMessageId:string,executionRecordId:string,outcomeRef:"answered"|"safe_fallback",at:string){const index=this.values.findIndex(value=>value.status==="pending"&&value.webChatSessionId===webChatSessionId&&value.conversationId===conversationId);if(index<0)return null;const value={...this.values[index]!,status:"succeeded" as const,inboundMessageId,executionRecordId,outcomeRef,completedAt:at};this.values[index]=value;return value;}
  public async failForSession(webChatSessionId:string,conversationId:string,at:string){const index=this.values.findIndex(value=>value.status==="pending"&&value.webChatSessionId===webChatSessionId&&value.conversationId===conversationId);if(index<0)return null;const value={...this.values[index]!,status:"failed" as const,completedAt:at,failureCode:"runtime_failure" as const};this.values[index]=value;return value;}
  public async expire(id:string,at:string){const index=this.values.findIndex(value=>value.id===id&&value.status==="pending");if(index<0)return null;const value={...this.values[index]!,status:"expired" as const,completedAt:at};this.values[index]=value;return value;}
}
...
test("EPIC054 PASS4 derives stage metadata and verification from durable facts",async()=>{
  ...
  const service=new ActivationService({findById:async()=>({})} as never,{listByCompany:async()=>[connection],findActiveById:async()=>connection} as never,{get:async()=>pilot} as never,attempts,clock);
  ...
  const started=await service.startVerification(context,1);
  assert.equal(await service.claimPublicVerification(connection.publicId,started.token,{sessionId:"wcs_00000000000000000000000000000000",conversationId:"cnv_00000000000000000000000000000000"}),true);
  await service.succeedForTurn({sessionId:"wcs_00000000000000000000000000000000",conversationId:"cnv_00000000000000000000000000000000"},"cmsg_00000000000000000000000000000000","aer_1","answered");
  const retry=await service.startVerification(context,1);
  assert.equal(await service.claimPublicVerification(connection.publicId,retry.token,{sessionId:"wcs_11111111111111111111111111111111",conversationId:"cnv_11111111111111111111111111111111"}),true);
  await service.failForSession({sessionId:"wcs_11111111111111111111111111111111",conversationId:"cnv_11111111111111111111111111111111"});
  assert.equal((await service.projection(context,1)).stages.find(stage=>stage.id==="verification")?.state,"complete");
});

test("EPIC054 PASS4 binds an opaque verification token to one session and stays pending until a persisted turn succeeds",async()=>{
  const attempts=new Attempts(),clock={now:()=>"2026-01-01T00:00:00.000Z"},connection={id:"wcc_1",publicId:"wc_public",status:"active" as const};
  const service=new ActivationService({findById:async()=>({name:"Company"})} as never,{listByCompany:async()=>[connection],findActiveById:async(id:string)=>id===connection.id?connection:null} as never,{} as never,attempts,clock);
  const started=await service.startVerification(context,1);
  assert.match(started.token,/^[A-Za-z0-9_-]{43}$/);
  assert.equal(attempts.values[0]?.tokenDigest===started.token,false);
  assert.equal(await service.canClaimPublicVerification(connection.publicId,started.token),true);
  assert.equal(await service.claimPublicVerification(connection.publicId,started.token,{sessionId:"wcs_00000000000000000000000000000000",conversationId:"cnv_00000000000000000000000000000000"}),true);
  ...
  assert.equal(await service.claimPublicVerification(connection.publicId,started.token,{sessionId:"wcs_11111111111111111111111111111111",conversationId:"cnv_11111111111111111111111111111111"}),false);
  await service.succeedForTurn({sessionId:"wcs_00000000000000000000000000000000",conversationId:"cnv_00000000000000000000000000000000"},"cmsg_00000000000000000000000000000000","aer_1","answered");
...
test("EPIC054 PASS4 lets an ordinary public Web Chat neither claim nor settle a verification",async()=>{
  const attempts=new Attempts(),clock={now:()=>"2026-01-01T00:00:00.000Z"},connection={id:"wcc_1",publicId:"wc_public",status:"active" as const};
  const service=new ActivationService({findById:async()=>({name:"Company"})} as never,{listByCompany:async()=>[connection],findActiveById:async()=>connection} as never,{} as never,attempts,clock);
  const started=await service.startVerification(context,1);
  assert.equal(await service.claimPublicVerification("wc_other",started.token,{sessionId:"wcs_00000000000000000000000000000000",conversationId:"cnv_00000000000000000000000000000000"}),false);
  assert.equal(attempts.values[0]?.status,"pending");
  await service.succeedForTurn({sessionId:"wcs_other",conversationId:"cnv_other"},"cmsg_other","aer_other","answered");
  assert.equal(attempts.values[0]?.status,"pending");
  assert.equal(await service.claimPublicVerification(connection.publicId,started.token,{sessionId:"wcs_00000000000000000000000000000000",conversationId:"cnv_00000000000000000000000000000000"}),true);
  await service.failForSession({sessionId:"wcs_00000000000000000000000000000000",conversationId:"cnv_00000000000000000000000000000000"});
