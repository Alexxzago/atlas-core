...
test("provides an accessible shell navigation and skip link", () => {
  ...
  render(<I18nProvider><AppShell route={{ name: "dashboard" }} workspace={null} workspaces={[]} companies={[]} selectedCompany={null} email="operator@example.test" onNavigate={navigate} onSelectWorkspace={() => {}} onSelectCompany={() => {}} onPassword={() => {}} onLogout={() => {}}><h1>Content</h1></AppShell></I18nProvider>);
...
test("keeps the closed drawer out of the keyboard flow and restores focus after Escape", async () => {
  render(<I18nProvider><AppShell route={{ name: "dashboard" }} workspace={null} workspaces={[]} companies={[]} selectedCompany={null} email="operator@example.test" onNavigate={() => {}} onSelectWorkspace={() => {}} onSelectCompany={() => {}} onPassword={() => {}} onLogout={() => {}}><h1>Content</h1></AppShell></I18nProvider>);
...
test("keeps the localized company switcher available in shell context", () => {
  ...
  render(<I18nProvider><AppShell route={{ name: "company-overview", companyId: 2 }} workspace={{ id: "workspace", name: "Workspace", role: "owner", capabilities: [] }} workspaces={[{ id: "workspace", name: "Workspace", role: "owner", capabilities: [] }]} companies={[{ id: 2, name: "Company Two", website: "", phone: "", email: "", status: "ready", createdAt: "" }]} selectedCompany={{ id: 2, name: "Company Two", website: "", phone: "", email: "", status: "ready", createdAt: "" }} email="operator@example.test" onNavigate={() => {}} onSelectWorkspace={() => {}} onSelectCompany={(id) => selected.push(id)} onPassword={() => {}} onLogout={() => {}}><h1>Content</h1></AppShell></I18nProvider>);
