...
function safe(handler:RequestHandler):RequestHandler { return async(req,res,next)=>{ try { await handler(req,res,next); } catch { res.status(400).json({ error:{ code:"validation_failed", message:"Billing request is invalid." } }); } }; }

export interface BillingRequestContext { readonly workspaceId:number; readonly callerIdentityId:string; }
export function createBillingControllers(service:BillingApplicationService):Record<"summary"|"entitlements"|"catalog"|"payerIdentityOptions"|"setPayerIdentity"|"clearPayerIdentity"|"checkout"|"portal"|"cancel"|"reactivate",(context:BillingRequestContext)=>RequestHandler> { return {
  ...
  checkout:context=>safe(async(req,res)=>{ const body=exact(req.body,["catalogEntryId"]), result=await service.checkout(context.workspaceId,catalogEntryId(body.catalogEntryId),idempotencyKey(req.headers["idempotency-key"])); respond(res,result); }),
  portal:context=>safe(async(req,res)=>{ exact(req.body,[]); respond(res,await service.portal(context.workspaceId)); }),
  cancel:context=>safe(async(req,res)=>{ exact(req.body,[]); respond(res,await service.cancel(context.workspaceId,idempotencyKey(req.headers["idempotency-key"]))); }),
  reactivate:context=>safe(async(req,res)=>{ exact(req.body,[]); respond(res,await service.reactivate(context.workspaceId,idempotencyKey(req.headers["idempotency-key"]))); }),
}; }
