...
export function createListConversationController(service: ConversationService, context: WorkspaceContext, actor: ActorContext): RequestHandler {
  return (req, res): void => { try { res.json(service.listInbox(context, req.params.companyId, actor.userId).map(inbox)); } catch (error: unknown) { respond(res, error); } };
}
...
function detail(value: ConversationDetailProjection) { return { ...inbox(value), messages: value.messages.map((message) => ({ messageId: message.messageId, participant: message.participant, deliveryCategory: message.deliveryCategory, content: message.content, createdAt: message.createdAt, delivery: lifecycle(message.delivery), voiceAvailable: message.voiceAvailable })) }; }
