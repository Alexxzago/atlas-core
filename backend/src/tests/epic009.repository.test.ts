...
test("migration 7 creates the constrained indexed schema without bootstrap profiles", () => {
  ...
  assert.equal((db.prepare("SELECT COUNT(*) AS count FROM schema_migrations").get() as { count: number }).count, 12);
...
test("migration 7 upgrades schema 6 data, restarts safely, cascades, and rolls back on failure", () => {
  ...
  try {
    ...
    assert.equal((restarted.prepare("SELECT COUNT(*) AS count FROM schema_migrations").get() as { count: number }).count, 12);
