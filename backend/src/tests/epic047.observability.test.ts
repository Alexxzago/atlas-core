...
test("EPIC047 PASS2 worker cycles receive unique non-durable run IDs", async () => {
  ...
  try { const one = createRunId(), two = createRunId(); assert.notEqual(one, two); await withRunContext(one, async () => { operationalLogger.info("worker_probe", { worker: "test" }); }); runtime.start(); await new Promise<void>((resolve) => setImmediate(resolve)); scheduled!(); await new Promise<void>((resolve) => setImmediate(resolve)); await runtime.stop(); const runs = logs.records.filter((value) => value.event === "worker_cycle_completed").map((value) => value.runId); assert.ok(runs.length >= 1); assert.ok(runs.every((value) => typeof value === "string" && String(value).startsWith("run_"))); assert.equal(currentOperationalContext().runId, undefined); } finally { logs.restore(); }
});
