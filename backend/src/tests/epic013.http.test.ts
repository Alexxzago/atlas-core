...
test("operational route authorizes before route-local parsing", async () => {
  ...
  const empty = express.Router(), app = createApp({ authorizedCompaniesRouter, chatRouter: empty, companiesRouter: empty, identityRouter: empty, knowledgeRouter: empty, publicWebChatRouter: empty, scrapeRouter: empty, workspacesRouter: empty }, { production: true, trustedLocalMode: false });
