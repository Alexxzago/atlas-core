...
export function Radio({ className, ...props }: InputHTMLAttributes<HTMLInputElement>): React.JSX.Element { return <input {...props} type="radio" className={classes("ds-radio", className)} />; }
...
export function Tabs({ tabs, selectedId, onSelect, label, className }: { readonly tabs: readonly TabDefinition[]; readonly selectedId: string; readonly onSelect: (id: string) => void; readonly label: string; readonly className?: string }): React.JSX.Element { return <div aria-label={label} className={classes("ds-tabs", className)} role="tablist">{tabs.map((tab) => <button aria-selected={tab.id === selectedId} className="ds-tab" disabled={tab.disabled} key={tab.id} role="tab" type="button" onClick={() => onSelect(tab.id)}>{tab.label}</button>)}</div>; }
