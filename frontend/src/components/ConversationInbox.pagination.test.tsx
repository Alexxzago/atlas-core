...
const first={conversationId:"first",channel:"whatsapp" as const,state:"open" as const,controlState:"automated" as const,controlledByCurrentActor:false,attentionReason:null,takenAt:null,releasedAt:null,lastOperatorActivityAt:null,resolvedAt:null,controlVersion:1,updatedAt:"2026-01-02T00:00:00Z",contactLabel:"First customer",participant:"First customer",preview:"First",deliveryCategory:"received" as const,lastActivityAt:"2026-01-02T00:00:00Z",delivery:null,unreadCount:12};
const second={...first,conversationId:"second",contactLabel:"Second customer",participant:"Second customer",preview:"Second",lastActivityAt:"2026-01-01T00:00:00Z",unreadCount:0};
...
test("appends cursor pages and marks the selected conversation read without a reload",async()=>{
  ...
  expect((await screen.findByText("12")).className).toContain("conversation-unread");
  fireEvent.click(screen.getByText("First customer"));
