...
export class VoiceMediaUploadWorkerService {
  public constructor(private readonly voices: VoiceRepositoryPort, private readonly media: Pick<MediaService, "open">, private readonly provider: WhatsAppOutboundMediaUploadPort, private readonly clock: { now(): string }, private readonly options: VoiceMediaUploadWorkerOptions) {}
  public async runOnce(context: WorkspaceContext, companyId: number): Promise<readonly VoiceMediaUploadWorkerOutcome[]> { const now = this.clock.now(), uploads = this.voices.leaseUploads(context, companyId, { owner: this.options.owner, now, expiresAt: new Date(Date.parse(now) + this.options.leaseMilliseconds).toISOString(), limit: this.options.batchSize }); return Promise.all(uploads.map(upload => this.process(context, companyId, upload))); }
  private async process(context: WorkspaceContext, companyId: number, upload: WhatsAppOutboundMediaUpload): Promise<VoiceMediaUploadWorkerOutcome> {
    const authorization = this.voices.authorizeUpload(context, companyId, upload.id, this.options.owner, this.clock.now());
