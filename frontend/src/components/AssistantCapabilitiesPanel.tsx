...
export function AssistantCapabilitiesPanel({ csrf, workspaceId, companyId, profileId, canManage }: Props): React.JSX.Element {
  ...
  return <section className="assistant-capabilities" aria-labelledby="assistant-capabilities-title">
    ...
    <div className="assistant-capabilities__list">{catalog.capabilities.map((item) => {
      const assigned = proposed.has(item.id), detail = presentation[item.id];
      return <article key={item.id} className="assistant-capability-card"><div><h3>{t(detail?.name ?? "capabilities.generic.name")}</h3><p>{t(detail?.purpose ?? "capabilities.generic.purpose")}</p><p className={`assistant-capability-card__availability assistant-capability-card__availability--${item.availability}`}><span>{t("capabilities.status")}</span><strong>{t(`capabilities.${item.availability}`)}</strong></p><p className="assistant-capability-card__cue">{item.consequence === "consequential" ? t("capabilities.consequentialCue") : t("capabilities.readOnlyCue")}</p>{item.safeReason && <p className="assistant-capability-card__guidance">{item.safeReason}</p>}{item.safeNextAction && <p className="assistant-capability-card__guidance">{item.safeNextAction}</p>}</div><label htmlFor={`capability-${item.id}`}><input id={`capability-${item.id}`} type="checkbox" checked={assigned} disabled={!canManage || pending} onChange={() => setProposed((current) => { const next = new Set(current); if (next.has(item.id)) next.delete(item.id); else next.add(item.id); return next; })}/>{t("capabilities.enabled")}</label></article>;
    })}</div>
    {canManage && <div className="action-row"><button className="button button--primary" type="button" disabled={pending || !changed} onClick={() => requiresConfirmation ? setConfirm(true) : void save()}>{pending ? t("capabilities.saving") : t("capabilities.save")}</button></div>}
    {confirm && <div className="confirm-panel" role="alertdialog" aria-label={t("capabilities.confirmLabel")}><p>{t("capabilities.confirm")}</p><div className="action-row"><button className="button button--primary" type="button" disabled={pending} onClick={() => void save()}>{t("capabilities.confirmSave")}</button><button className="button button--secondary" type="button" disabled={pending} onClick={() => setConfirm(false)}>{t("common.cancel")}</button></div></div>}
  </section>;
}
