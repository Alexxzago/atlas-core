...
export class SessionRepository implements SessionRepositoryPort {
  ...
  public rotateCsrf(id:string,expectedGeneration:number,csrfDigest:string,at:string,idleExpiresAt:string):boolean{return this.db.prepare("UPDATE sessions SET csrf_digest=?,csrf_generation=csrf_generation+1,last_seen_at=?,idle_expires_at=? WHERE id=? AND state='active' AND csrf_generation=?").run(csrfDigest,at,idleExpiresAt,id,expectedGeneration).changes===1;}
