...
function AuthenticatedCompanyPortalContent({ csrf, userId, email, isPlatformAdmin, onPassword, onLogout }: Props): React.JSX.Element {
  ...
  const assistantSection=route.name==="company-assistant-section"&&(route.section==="general"||route.section==="behavior"||route.section==="capabilities")?route.section:undefined;
  const assistantPanel = assistantSection==="capabilities"&&selectedProfile?<AssistantCapabilitiesPanel csrf={csrf} workspaceId={state.selectedWorkspace?.id??null} companyId={state.selectedCompanyId} profileId={selectedProfile.id} canManage={(state.selectedWorkspace?.capabilities??[]).includes("assistant:capability:manage")}/>:<AssistantProfilesPanel csrf={csrf} workspaceId={state.selectedWorkspace?.id ?? null} workspaceRole={state.selectedWorkspace?.role ?? null} capabilities={state.selectedWorkspace?.capabilities ?? []} companyId={state.selectedCompanyId} companyName={selectedCompany?.name ?? null} companySelected={state.selectedCompanyId !== null} profiles={state.profiles} selectedProfile={selectedProfile} transientArchivedProfile={state.transientArchivedProfile} loading={state.profilesLoading} error={state.profileError} formMode={state.formMode} submitting={state.submitting} transitionTarget={state.transitionTarget} {...(assistantSection ? { activeSection: assistantSection } : {})} onNavigate={navigate} onSelectProfile={(id) => {void selectProfile(id);navigate(`/companies/${state.selectedCompanyId}/assistant/${id}/general`);}} onOpenCreate={() => openProfileForm("create")} onOpenEdit={() => openProfileForm("edit")} onCloseForm={closeProfileForm} onSubmitForm={(input) => void submitProfile(input)} onTransition={(profile, target) => void transitionProfile(profile, target)} onRetry={reloadProfiles}/>;
