...
test("WhatsApp validation classifies Graph outcomes and logs only sanitized diagnostics", async () => {
  ...
  const valid = new WhatsAppCloudApiProvider("secret-token", "v26.0", async (url, init) => { requested.push({ url: String(url), authorization: new Headers(init?.headers).get("authorization") }); return new Response(JSON.stringify({ id: "phone-id", whatsapp_business_account: { id: "waba-id" } }), { status: 200 }); }, value => diagnostics.push(value));
  assert.deepEqual(await valid.validateConnection({ accessToken: "secret-token", phoneNumberId: "phone-id", whatsappBusinessAccountId: "waba-id" }), { status: "valid" }); assert.equal(requested[0]?.url, "https://graph.facebook.com/v26.0/phone-id?fields=id,whatsapp_business_account"); assert.equal(requested[0]?.authorization, "Bearer secret-token");
  for (const [httpStatus, failureCode] of [[401, "invalid_credentials"], [403, "insufficient_permissions"], [404, "phone_number_not_found"], [429, "rate_limited"], [500, "provider_unavailable"], [504, "provider_timeout"], [400, "provider_rejected"]] as const) { const provider = new WhatsAppCloudApiProvider("secret-token", "v26.0", async () => new Response(JSON.stringify({ error: { code: 123, error_subcode: 456, type: "OAuthException", is_transient: false, message: "secret message" } }), { status: httpStatus }), value => diagnostics.push(value)); assert.deepEqual(await provider.validateConnection({ accessToken: "secret-token", phoneNumberId: "phone-id", whatsappBusinessAccountId: "waba-id" }), { status: "invalid", failureCode }); }
  const mismatch = new WhatsAppCloudApiProvider("secret-token", "v26.0", async () => new Response(JSON.stringify({ id: "phone-id", whatsapp_business_account: { id: "other" } }), { status: 200 })); assert.deepEqual(await mismatch.validateConnection({ accessToken: "secret-token", phoneNumberId: "phone-id", whatsappBusinessAccountId: "waba-id" }), { status: "invalid", failureCode: "business_account_mismatch" });
  ...
  for (const entry of diagnostics) { const json = JSON.stringify(entry); assert.deepEqual(Object.keys(entry as object).sort(), ["errorType", "event", "graphApiVersion", "httpStatus", "operation", "providerCode", "providerSubcode", "timestamp", "transient"]); assert.equal(json.includes("secret-token") || json.includes("phone-id") || json.includes("waba-id") || json.includes("secret message"), false); }
});
