...
function item(overrides:Partial<ConversationInboxItem>={}):ConversationInboxItem{return {conversationId:"internal-id",channel:"web_chat",state:"open",controlState:"automated",attentionReason:null,takenAt:null,releasedAt:null,lastOperatorActivityAt:null,resolvedAt:null,controlVersion:1,updatedAt:"2026-01-01T00:00:00Z",contactLabel:"Web chat visitor",participant:null,preview:"Hello",deliveryCategory:"received",lastActivityAt:"2026-01-01T00:00:00Z",delivery:null,...overrides}}
test("builds safe list identity without promoting internal IDs",()=>{const [view]=buildConversationInboxViewModel([item()]);assert.equal(view?.identity,"Web chat visitor");assert.equal(view?.id,"internal-id")});
...
test("keeps attention independent from Atlas responder state",()=>{const [view]=buildConversationInboxViewModel([item({controlState:"human_required"})]);assert.deepEqual([view?.state,view?.attentionRequired], ["attention",true])});
