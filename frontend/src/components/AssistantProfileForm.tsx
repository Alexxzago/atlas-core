    ...
    <fieldset className="assistant-form-fields" disabled={submitting}>
      <section className="assistant-form-section">
        ...
        <div className="assistant-form-grid">
          ...
          <label className="form-field assistant-form-wide" htmlFor={field("businessRole")}><span>{t("profiles.field.businessRole")}</span><input id={field("businessRole")} maxLength={120} value={values.businessRole} onChange={(event) => set("businessRole", event.target.value)} /></label>
        </div>
      </section>
      <section className="assistant-form-section assistant-form-section--optional">
        ...
        <div className="assistant-form-grid">
          ...
          <label className="form-field assistant-form-wide" htmlFor={field("description")}><span>{t("profiles.field.description")}</span><textarea id={field("description")} maxLength={240} value={values.description} onChange={(event) => set("description", event.target.value)} /></label>
        </div>
      </section>
      {disclosure(`${prefix}-purpose`, t("profiles.form.purpose"), t("profiles.form.optionalDescription"), purposeOpen, () => setPurposeOpen((open) => !open), <>
        <label className="form-field assistant-form-wide" htmlFor={field("objective")}><span>{t("profiles.field.objective")}</span><textarea id={field("objective")} maxLength={500} value={values.objective} onChange={(event) => set("objective", event.target.value)} /></label>
        <label className="form-field assistant-form-wide" htmlFor={field("audience")}><span>{t("profiles.field.audience")}</span><textarea id={field("audience")} maxLength={300} value={values.audience} onChange={(event) => set("audience", event.target.value)} /></label>
      </>)}
      {disclosure(`${prefix}-messages`, t("profiles.form.messages"), mode === "edit" ? t("profiles.form.messagesEditDescription") : t("profiles.form.optionalDescription"), messagesOpen, () => setMessagesOpen((open) => !open), <>
        <label className="form-field assistant-form-wide" htmlFor={field("welcomeMessage")}><span>{t("profiles.field.welcomeMessage")}</span><textarea id={field("welcomeMessage")} maxLength={500} value={values.welcomeMessage} onChange={(event) => set("welcomeMessage", event.target.value)} /></label>
        <label className="form-field assistant-form-wide" htmlFor={field("fallbackMessage")}><span>{t("profiles.field.fallbackMessage")}</span><textarea id={field("fallbackMessage")} maxLength={500} required={mode === "edit"} value={values.fallbackMessage} onChange={(event) => set("fallbackMessage", event.target.value)} /></label>
      </>)}
    </fieldset>
    {error && <p id={errorId} className="inline-message inline-message--error" role="alert">{error}</p>}
    <div className="assistant-form-actions"><p>{t("profiles.form.requiredHint")}</p><div className="action-row"><button className="button button--primary" disabled={submitting}>{submitting ? t("common.saving") : t("profiles.save")}</button><button className="button button--secondary" type="button" disabled={submitting} onClick={onCancel}>{t("common.cancel")}</button></div></div>
  </form>;
}
