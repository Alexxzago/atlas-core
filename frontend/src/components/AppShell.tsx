...
export function PageHeader({ title, description, trail }: { readonly title: string; readonly description?: string; readonly trail?: string }): React.JSX.Element { return <div className="ds-page-header-with-trail">{trail && <p className="ds-page-header__trail">{trail}</p>}<DesignSystemPageHeader title={title} {...(description === undefined ? {} : { description })}/></div>; }
...
export function AppShell(props: AppShellProps): React.JSX.Element {
  ...
  const dismissChooser = useCallback(() => { closeChooser(); if (props.route.name === "companies") props.onNavigate("/dashboard"); }, [closeChooser, props.onNavigate, props.route.name]);
  ...
  return <div className={`app-shell authenticated-portal${props.selectedCompany ? " has-company" : " no-company"}`}><SkipLink />
    <aside className="app-sidebar" aria-label={t("shell.applicationNavigation")}><div className="app-sidebar__brand"><span aria-hidden="true"/><strong>ATLAS</strong></div>{companyContext}{props.workspace && <p className="workspace-context">{props.workspace.name}</p>}{navigation}<Button className="workspace-menu-trigger" variant="quiet" aria-expanded={accountOpen} onClick={(event) => { accountTrigger.current = event.currentTarget; setAccountOpen((value) => !value); }}>{t("shell.workspaceMenu")}</Button></aside>
    <header className="mobile-context-bar"><Button ref={mobileTrigger} className="mobile-navigation-trigger" variant="secondary" aria-expanded={mobileOpen} aria-controls="mobile-navigation" onClick={() => setMobileOpen(true)}>{t("shell.openNavigation")}</Button>{companyContext}<Button className="mobile-account-trigger" variant="secondary" aria-label={t("shell.workspaceMenu")} onClick={(event) => { accountTrigger.current = event.currentTarget; setAccountOpen((value) => !value); }}>•••</Button></header>
...
function AccountMenu({ ref, style, ...props }: AppShellProps & { readonly navigate: Navigate; readonly ref: React.Ref<HTMLDivElement>; readonly style: React.CSSProperties }): React.JSX.Element {
  ...
  return <div ref={ref} style={style} className="workspace-menu" role="dialog" aria-modal="false" aria-label={t("shell.workspaceMenu")}>
    <section><p>{t("shell.menu.workspace")}</p><strong>{props.workspace?.name ?? t("shell.noWorkspace")}</strong>{props.workspaces.length > 1 && <Button variant="quiet" onClick={() => props.navigate("/settings")}>{t("shell.changeWorkspace")}</Button>}{props.companies.length > 1 && <Button variant="quiet" onClick={() => props.navigate("/companies")}>{t("shell.changeCompany")}</Button>}<Button variant="quiet" onClick={() => props.navigate("/billing")}>{t("billing.title")}</Button><Button variant="quiet" onClick={() => props.navigate("/settings")}>{t("shell.workspaceSettings")}</Button><Button variant="quiet" onClick={() => props.navigate("/dashboard")}>{t("shell.backToDashboard")}</Button></section>
    <section><p>{t("shell.menu.appearance")}</p><ThemeSelector/></section>
    <LanguageControl/>
    <section><p>{t("shell.menu.account")}</p><small>{props.email}</small>{props.isPlatformAdmin&&<Button variant="quiet" onClick={() => props.navigate("/admin")}>Volver a Administración</Button>}<Button variant="quiet" disabled={props.logoutPending} onClick={props.onPassword}>{t("portal.password")}</Button><Button variant="danger" disabled={props.logoutPending} aria-busy={props.logoutPending} onClick={() => void props.onLogout()}>{props.logoutPending ? "Cerrando sesión..." : t("shell.signOut")}</Button>{props.logoutError&&<p className="inline-message inline-message--error" role="alert">{props.logoutError}</p>}</section>
  </div>;
}
