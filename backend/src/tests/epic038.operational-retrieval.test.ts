...
function setup() {
  ...
  const add = async (sourceId: string, revisionId: string, text: string): Promise<void> => {
    ...
    await indexing.indexCompletedRevision(context, company.id, { sourceId, sourceRevisionId: revisionId, contentDigest: `digest_${revisionId}`, normalizedText: text, completedAt: now });
  };
  return { database, context, company, repository, retrieval, add };
}

test("EPIC038 ranks deterministically, deduplicates, and enforces chunk and UTF-8 byte budgets", async () => {
  const value = setup();
  try {
    await value.add("source_a", "revision_a", "central park central park alpha");
    await value.add("source_b", "revision_b", "central park beta");
    const ranked = await value.retrieval.retrieve(value.context, value.company.id, ["revision_b", "revision_a", "revision_a"], "CENTRAL PARK", 1);
    ...
    assert.deepEqual(await value.retrieval.retrieve(value.context, value.company.id, ["revision_a"], null as unknown as string), []);
    assert.deepEqual(value.retrieval.assemble(ranked, 0), { text: "", citations: [] });
  } finally { value.database.close(); }
});

test("EPIC038 reads only active published-source revisions and rebuilding does not mutate prior chunks", async () => {
  const value = setup();
  try {
    await value.add("source_a", "revision_a", "A is active and searchable");
    await value.add("source_b", "revision_b", "B is active and searchable");
    ...
    assert.deepEqual(await value.retrieval.retrieve(value.context, value.company.id, ["revision_a"], "active"), []);
    assert.equal((await value.retrieval.retrieve(value.context, value.company.id, ["revision_b"], "active"))[0]!.sourceRevisionId, "revision_b");
...
test("EPIC038 failed reindex preserves the previously ready publication index", async () => {
  const value = setup();
  try {
    await value.add("source_a", "revision_a", "The active publication remains searchable.");
