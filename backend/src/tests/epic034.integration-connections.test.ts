...
test("EPIC042 reserved Integration Connection creation persists row, secret, and reset state atomically", async () => { const database = createDatabase(":memory:"), context = createWorkspaceContext(new WorkspaceRepository(database).resolveDefault()), company = new CompanyRepository(database).create(context, { name: "Reserved", website: "https://reserved.test" }), repository = new IntegrationConnectionRepository(new LocalSqlDatabase(database)), service = new IntegrationConnectionService(repository, new AesGcmIntegrationSecretCipher({activeKeyId:"active_2026",activeKey:Buffer.alloc(32,8),previousKeyId:"previous_2025",previousKey:Buffer.alloc(32,9)}), { validate: async () => ({ status: "valid" }) }, new Clock()), id = "inc_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" as never, configuration = { wabaId: "123", phoneNumberId: "456", graphApiVersion: "v25.0" }; try { const created = await service.createWithReservedId(context, company.id, id, { provider: "meta_whatsapp", kind: "cloud_api", configuration, plaintextSecret: "meta-token" }); assert.equal(created.kind, "created"); assert.deepEqual(created.connection.configuration, configuration); const encrypted = await repository.findSecret(context, company.id, id), state = await repository.findState(context, company.id, id); assert.ok(encrypted); assert.match(encrypted!,/^v2\.active_2026\./u); assert.equal(encrypted!.includes("meta-token"), false); assert.equal(state?.validationState, "not_validated"); assert.equal(state?.healthState, "inactive"); assert.equal((await service.createWithReservedId(context, company.id, id, { provider: "meta_whatsapp", kind: "cloud_api", configuration, plaintextSecret: "different-token" })).kind, "replayed"); await assert.rejects(() => service.createWithReservedId(context, company.id, id, { provider: "meta_whatsapp", kind: "cloud_api", configuration: { ...configuration, phoneNumberId: "789" }, plaintextSecret: "meta-token" }), IntegrationConnectionConflictError); } finally { database.close(); } });
