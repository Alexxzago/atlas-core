...
export function ConfirmDialog({ open, title, description, cancelLabel, confirmLabel, onCancel, onConfirm, closeOnBackdropClick = false }: { readonly open: boolean; readonly title: string; readonly description?: string; readonly cancelLabel: string; readonly confirmLabel: string; readonly onCancel: () => void; readonly onConfirm: () => void; readonly closeOnBackdropClick?: boolean }): React.JSX.Element | null {
  ...
  return createPortal(<div aria-hidden="false" className="ds-dialog-backdrop" onMouseDown={(event) => { if (closeOnBackdropClick && event.target === event.currentTarget) onCancel(); }}><div aria-describedby={description ? descriptionId : undefined} aria-labelledby={titleId} aria-modal="true" className="ds-dialog" ref={dialogRef} role="alertdialog" tabIndex={-1} onKeyDown={onKeyDown}><header className="ds-dialog__header"><h2 className="ds-dialog__title" id={titleId}>{title}</h2>{description && <p className="ds-dialog__description" id={descriptionId}>{description}</p>}</header><div className="ds-dialog__actions"><Button ref={cancelRef} variant="secondary" onClick={onCancel}>{cancelLabel}</Button><Button variant="danger" onClick={onConfirm}>{confirmLabel}</Button></div></div></div>, document.body);
}
