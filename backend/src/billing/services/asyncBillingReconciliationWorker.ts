...
export class AsyncBillingReconciliationWorker {
  ...
  public async runNext(owner = `billing-reconciliation-${randomUUID()}`): Promise<BillingReconciliationRunResult> {
    ...
    const subscription = await this.work.trustedSubscription(claim), enrollment = subscription ? null : await this.work.trustedEnrollment(claim), reference = subscription?.provider_subscription_id ?? enrollment?.provider_subscription_id, provider = this.providers.get((subscription?.provider_kind ?? enrollment?.provider_kind) as "stripe" | "mercadopago");
    ...
    if (read.kind === "not_found") {
      ...
      return this.work.apply(claim, subscription!, evidence, "reconciliation_required", this.now());
    }
    ...
    return enrollment ? this.work.applyEnrollment(claim, enrollment, read.evidence, effectiveSubscriptionStateForEvidence(read.evidence), this.now()) : this.work.apply(claim, subscription!, read.evidence, effectiveSubscriptionStateForEvidence(read.evidence), this.now());
  }
