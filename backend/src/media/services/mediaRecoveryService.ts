...
export class MediaRecoveryService {
  ...
  public async reconcileReclaims(context: WorkspaceContext, companyId: number, owner: string, token: string, now: string, expiresAt: string, limit: number): Promise<number> {
    ...
    for (const blob of await this.repository.leaseReclaims(context,companyId,owner,token,now,expiresAt,limit)) {
      const result=await this.storage.delete(blob.storageReference);
      if (result.status === "absent" && await this.repository.finalizeLeasedReclaim(context,companyId,blob.id,owner,token,this.clock.now())) completed+=1;
    }
    return completed;
  }

  private async reconcileReady(context: WorkspaceContext, companyId: number, limit: number): Promise<void> { for(const value of await this.repository.listReadyBlobs(context,companyId,limit)){try{await this.exactBlob(value);}catch(error:unknown){const category=error instanceof MediaStorageError?error.category:error instanceof MediaDomainError&&error.code==="media_integrity_invalid"?"integrity":"unavailable";await this.repository.markBlobUnavailable(context,companyId,value.id,category,this.clock.now());}} }
  /** 24 hours is the initial operational safety threshold for metadata-less owned objects. */
  private async cleanupOrphans(context: WorkspaceContext, companyId: number, limit: number): Promise<void> { const cutoff=Date.now()-24*60*60*1000;for(const value of await this.storage.listOwned({workspaceId:context.workspaceId,companyId},limit)){if(Date.parse(value.createdAt)>cutoff||await this.repository.referencesStorage(context,companyId,value.reference))continue;await this.storage.delete(value.reference);} }
  ...
  private async recoverAttempt(context: WorkspaceContext, companyId: number, owner: string, token: string, attempt: MediaIngestAttempt): Promise<void> {
    try {
      ...
      await this.repository.settleRecovery(context,companyId,attempt.assetId,owner,token,blob(context,companyId,attempt),this.clock.now());
      void final;
      return;
    } catch (error: unknown) {
  ...
  private async fail(context: WorkspaceContext, companyId: number, attempt: MediaIngestAttempt, owner: string, token: string, error: unknown): Promise<void> { const category=error instanceof MediaStorageError?error.category:error instanceof MediaDomainError&&error.code==="media_integrity_invalid"?"integrity":"transient_provider";await this.repository.failRecovery(context,companyId,attempt.assetId,owner,token,category,category!=="integrity"&&category!=="collision"&&category!=="permanent",this.clock.now()); }
}
