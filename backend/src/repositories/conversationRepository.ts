...
export class ConversationRepository implements ConversationRepositoryPort {
  ...
  public findConversationDetail(context: WorkspaceContext, companyId: number, id: ConversationId): ConversationDetailProjection | null {
    ...
    const messages = this.listMessages(context, companyId, id).map((value) => Object.freeze({ messageId: value.id, participant: "masked", deliveryCategory: deliveryCategory(value.direction), content: bounded(value.content, 4_000), createdAt: value.createdAt, delivery: this.deliveryProjection(context, companyId, value.id), voiceAvailable: this.hasVoiceReadModel(context, companyId, id, value.id) }));
