...
export function createListConversationController(service: ConversationService, context: WorkspaceContext, actor: ActorContext): RequestHandler {
  return (req, res): void => { try { const page = service.listInbox(context, req.params.companyId, actor.userId, req.query); res.json({ items: page.items.map(inbox), nextCursor: page.nextCursor }); } catch (error: unknown) { respond(res, error); } };
}
export function createGetConversationController(service: ConversationService, context: WorkspaceContext, actor: ActorContext): RequestHandler {
  return (req, res): void => { try { res.json(detail(service.detail(context, req.params.companyId, req.params.conversationId, actor.userId))); } catch (error: unknown) { respond(res, error); } };
}
export function createMarkConversationReadController(service: ConversationService, context: WorkspaceContext, actor: ActorContext): RequestHandler { return (req, res): void => { try { service.markRead(context, req.params.companyId, req.params.conversationId, actor.userId); res.status(204).end(); } catch (error: unknown) { respond(res, error); } }; }
