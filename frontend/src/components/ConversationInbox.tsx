...
export function ConversationInbox({ csrf, workspaceId, companyId, capabilities }: Props): React.JSX.Element {
  ...
  const send = async (): Promise<void> => {
    ...
    mutationInFlight.current = true; mutationAbort.current?.abort(); const controller = new AbortController(); mutationAbort.current = controller; setWorking(true); setDetailError(false);
    try { await atlasApi.sendConversationMessage(csrf, workspaceId, companyId, selected.conversationId, submitted, crypto.randomUUID()); setContent(""); scrollToLatest(); await loadDetail(selected.conversationId); }
...
function mergeDetail(detail: ConversationDetail, current: ConversationDetail): ConversationDetail { return current.controlVersion >= detail.controlVersion ? { ...detail, ...authority(current) } : detail; }
