...
export function AuthenticationPortal(): React.JSX.Element {
  ...
  return <AuthForm title={es ? "Iniciar sesión" : "Log in"} error={error} onSubmit={event => { event.preventDefault(); void submit(async () => { const result = await atlasApi.login(email, password); setCsrf(result.csrfToken); setIdentity(await atlasApi.currentIdentity()); setView(invitationLink ? "invitation" : "authenticated"); }); }}><label>{es ? "Correo electrónico" : "Email"}<input type="email" autoComplete="username" required value={email} onChange={event => setEmail(event.target.value)}/></label><Password value={password} onChange={setPassword} label={es ? "Contraseña" : "Password"}/><button>{es ? "Ingresar" : "Log in"}</button><button type="button" onClick={() => setView("request")}>{es ? "Crear contraseña" : "Enroll password"}</button></AuthForm>;
}
