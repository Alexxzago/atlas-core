...
class LibsqlRestoredDatabaseAccess implements RestoredDatabaseAccess { public constructor(private readonly database:SqlDatabase,private readonly whatsApp:RotationCipher,private readonly integration:RotationCipher){} public async verify(migrationHead:string):Promise<void>{await new DisasterRecoveryRepository(this.database).verifyReadOnly(migrationHead);await this.database.query("SELECT encrypted_access_token FROM whatsapp_connection_credentials LIMIT 1");await this.database.query("SELECT encrypted_secret FROM integration_connection_secrets LIMIT 1");} public async mediaInventory():Promise<readonly RestoredMediaEntry[]>{const rows=await this.database.query<{storage_reference:string;sha256_digest:string;size_bytes:number;content_type:string}>("SELECT storage_reference,sha256_digest,size_bytes,content_type FROM media_blobs WHERE state='active' ORDER BY rowid");return rows.map(row=>Object.freeze({storageReference:row.storage_reference,sizeBytes:Number(row.size_bytes),contentType:row.content_type,checksum:row.sha256_digest}));} public credentialCounts(){return new RestoredCredentialCompatibilityVerifier(this.database,this.whatsApp,this.integration).verify();} }
