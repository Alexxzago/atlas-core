...
test("EPIC044 upgrades 0059 data without changing delivery order or event cursors", () => {
  ...
  try {
    ...
    assert.deepEqual({ ...(reopened.prepare("SELECT id,name FROM schema_migrations ORDER BY id DESC LIMIT 1").get() as Record<string, unknown>) }, { id: 72, name: "0072_conversation_delivery_feed_events" });
