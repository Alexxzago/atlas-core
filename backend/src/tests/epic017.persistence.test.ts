...
async function setup() {
  ...
  const companies = new CompanyRepository(database), first = await companies.create(primary, { name: "First", website: "https://first.test", status: "ready" }), second = await companies.create(secondary, { name: "Second", website: "https://second.test", status: "ready" });
  ...
  await profiles.create(primary, first.id, firstProfile); await profiles.create(secondary, second.id, secondProfile);
  ...
  assert.ok(await connectionRepository.create(primary, firstConnection)); assert.ok(await connectionRepository.create(secondary, secondConnection));
...
test("EPIC-017 Phase 1 persists tenant-scoped WhatsApp Connections and bindings", async () => {
  const value = await setup();
  try {
    assert.equal((await value.connectionRepository.findByPhoneNumberId("phone-first"))?.id, value.firstConnection.id);
    assert.equal(await value.connectionRepository.findById(value.secondary, value.first.id, value.firstConnection.id), null);
    assert.deepEqual(await value.connectionRepository.listByCompany(value.secondary, value.first.id), []);
    assert.equal(await value.connectionRepository.create(value.primary, reconstructWhatsAppConnection({ ...value.firstConnection, id: whatsAppConnectionId("wac_3123456789abcdef0123456789abcdef"), assistantProfileId: value.secondProfile.id, phoneNumberId: "phone-invalid-profile" })), null);
    ...
    assert.equal((await value.connectionRepository.updateStatus(value.primary, value.first.id, value.firstConnection.id, value.firstConnection.updatedAt, "inactive", later))?.status, "inactive");
    ...
    const firstConversation = await value.conversations.open(value.primary, value.first.id, "whatsapp"), firstCustomer = await value.conversations.addParticipant(value.primary, value.first.id, firstConversation.id, { type: "whatsapp_contact", reference: "sender-one" }), firstAssistant = await value.conversations.addParticipant(value.primary, value.first.id, firstConversation.id, { type: "assistant", reference: value.firstProfile.id });
    const secondConversation = await value.conversations.open(value.secondary, value.second.id, "whatsapp"), secondCustomer = await value.conversations.addParticipant(value.secondary, value.second.id, secondConversation.id, { type: "whatsapp_contact", reference: "sender-one" }), secondAssistant = await value.conversations.addParticipant(value.secondary, value.second.id, secondConversation.id, { type: "assistant", reference: value.secondProfile.id });
    ...
    assert.ok(await bindings.createBinding(firstBinding)); assert.ok(await bindings.createBinding(secondBinding));
    assert.equal((await bindings.findBinding(value.firstConnection.id, "sender-one"))?.conversationId, firstConversation.id);
...
test("EPIC-017 Phase 1 persists generic provider idempotency, message references, and outbound delivery uniqueness", async () => {
  const value = await setup();
  try {
    const conversation = await value.conversations.open(value.primary, value.first.id, "whatsapp"), customer = await value.conversations.addParticipant(value.primary, value.first.id, conversation.id, { type: "whatsapp_contact", reference: "sender" }), assistant = await value.conversations.addParticipant(value.primary, value.first.id, conversation.id, { type: "assistant", reference: value.firstProfile.id });
    const inbound = await value.conversations.addMessage(value.primary, value.first.id, conversation.id, { senderParticipantId: customer.id, direction: "inbound", content: "Question" });
    const outbound = await value.conversations.addMessage(value.primary, value.first.id, conversation.id, { senderParticipantId: assistant.id, direction: "outbound", content: "Answer" });
    ...
    assert.equal((await events.claim(event)).claimed, true); assert.equal((await events.claim({ ...event, id: "cpe_1123456789abcdef0123456789abcdef" as never })).claimed, false);
    assert.equal((await events.claim({ ...event, id: "cpe_2123456789abcdef0123456789abcdef" as never, transportProvider: "telegram" })).claimed, true);
    assert.equal((await events.updateState(event.id, "claimed", "processing", later))?.state, "processing");
    ...
    assert.ok(await records.create(inboundRecord)); assert.ok(await records.create(outboundRecord));
    assert.throws(() => records.attachExternalMessageId(outboundRecord.id, "wamid-one", later));
    assert.equal((await records.attachExternalMessageId(outboundRecord.id, "wamid-out", later))?.externalMessageId, "wamid-out");
    ...
    assert.ok(await deliveries.create(delivery)); assert.equal(await deliveries.create({ ...delivery, id: "odl_1123456789abcdef0123456789abcdef" as never }), null);
    assert.equal((await deliveries.findById(delivery.id))?.state, "pending");
    const leased = await deliveries.leaseReady("worker-one", at, "2026-07-27T12:01:00.000Z", 10);
    assert.equal(leased.length, 1); assert.equal(leased[0]?.state, "leased"); assert.equal(leased[0]?.attemptCount, 1);
    assert.equal((await deliveries.completeLease(delivery.id, "worker-one", "accepted", null, later))?.state, "accepted");
  } finally { value.database.close(); }
});

test("EPIC-017 captures an inbound provider event and message atomically for restart recovery", async () => {
  const value = await setup();
  try {
    const conversation = await value.conversations.open(value.primary, value.first.id, "whatsapp"), customer = await value.conversations.addParticipant(value.primary, value.first.id, conversation.id, { type: "whatsapp_contact", reference: "sender" });
    ...
    const first = await events.captureInbound(event, inbound, record), duplicate = await events.captureInbound({ ...event, id: "cpe_4123456789abcdef0123456789abcdef" as never }, inbound, record);
    assert.equal(first.claimed, true); assert.equal(duplicate.claimed, false); assert.equal(first.inbound.id, duplicate.inbound.id);
    assert.equal((await value.conversations.listMessages(value.primary, value.first.id, conversation.id)).length, 1);
    ...
    assert.equal((await events.listRecoverable("meta_whatsapp_cloud", 10))[0]?.conversationMessageId, inbound.id);
  } finally { value.database.close(); }
});

async function mediaCapture(value: Awaited<ReturnType<typeof setup>>, suffix: string, mime = "image/jpeg", ordinal = 0) {
  const conversation = await value.conversations.open(value.primary, value.first.id, "whatsapp"), customer = await value.conversations.addParticipant(value.primary, value.first.id, conversation.id, { type: "whatsapp_contact", reference: `sender-${suffix}` });
...
test("EPIC-040 captures media atomically and blocks its durable execution request", async () => {
  const value = await setup(); try { const input = await mediaCapture(value, "5123456789abcdef0123456789abcdef"), result = await new ChannelProviderEventRepository(value.database).captureInboundExecution(input.event, input.inbound, input.provider, input.request, [input.attachment]);
...
test("EPIC-040 rolls back capture when a later media ledger insert conflicts", async () => {
  const value = await setup(); try { const input = await mediaCapture(value, "6123456789abcdef0123456789abcdef"), divergent = { ...input.attachment, id: "wim_other", descriptor: { ...input.attachment.descriptor, declaredMime: "image/png" } };
...
test("EPIC-040 replays the same media capture without duplicate durable rows", async () => {
  const value = await setup(); try { const input = await mediaCapture(value, "7123456789abcdef0123456789abcdef"), events = new ChannelProviderEventRepository(value.database), first = await events.captureInboundExecution(input.event, input.inbound, input.provider, input.request, [input.attachment]), replay = await events.captureInboundExecution({ ...input.event, id: "cpe_8123456789abcdef0123456789abcdef" as never }, input.inbound, input.provider, input.request, [input.attachment]);
...
test("EPIC-040 rejects divergent media descriptor replay without mutating original", async () => {
  const value = await setup(); try { const input = await mediaCapture(value, "9123456789abcdef0123456789abcdef"), events = new ChannelProviderEventRepository(value.database); await events.captureInboundExecution(input.event, input.inbound, input.provider, input.request, [input.attachment]);
...
test("EPIC-040 preserves text-only capture request semantics", async () => {
  const value = await setup(); try { const input = await mediaCapture(value, "a123456789abcdef0123456789abcdef"), result = await new ChannelProviderEventRepository(value.database).captureInboundExecution(input.event, { ...input.inbound, content: "Hello" }, input.provider, input.request);
