...
function operatorConversationMessageResponse(value: unknown): OperatorConversationMessageResult { const response = record(value, "Operator conversation message"), message = record(response.message, "Operator conversation message"), delivery = record(response.delivery, "Operator conversation message"); if (Object.keys(response).length !== 3 || Object.keys(message).length !== 3 || Object.keys(delivery).length !== 2 || !["pending", "accepted", "uncertain"].includes(String(delivery.state)) || text(response.messageId, "Operator conversation message") !== text(message.messageId, "Operator conversation message")) throw malformedList("Operator conversation message"); return { messageId: response.messageId as string, message: { messageId: message.messageId as string, content: text(message.content, "Operator conversation message"), createdAt: timestamp(message.createdAt, "Operator conversation message") }, delivery: { id: text(delivery.id, "Operator conversation message"), state: delivery.state as OperatorConversationMessageResult["delivery"]["state"] } }; }
...
export const atlasApi = {
  ...
  releaseConversation:(csrf:string,workspaceId:string,companyId:number,conversationId:string,expectedVersion:number,operationId:string,signal?:AbortSignal):Promise<ConversationControlResponse>=>request(`/workspaces/${segment(workspaceId)}/companies/${segment(companyId)}/conversations/${segment(conversationId)}/release`,{method:"POST",headers:{"x-csrf-token":csrf},body:JSON.stringify({expectedVersion,operationId}),signal:signal??null}),
   ...
   resumeConversation:(csrf:string,workspaceId:string,companyId:number,conversationId:string,expectedVersion:number,operationId:string,signal?:AbortSignal):Promise<ConversationControlResponse>=>request(`/workspaces/${segment(workspaceId)}/companies/${segment(companyId)}/conversations/${segment(conversationId)}/resume`,{method:"POST",headers:{"x-csrf-token":csrf},body:JSON.stringify({expectedVersion,operationId}),signal:signal??null}),
    sendConversationMessage:async(csrf:string,workspaceId:string,companyId:number,conversationId:string,content:string,idempotencyKey:string):Promise<OperatorConversationMessageResult>=>operatorConversationMessageResponse(await request<unknown>(`/workspaces/${segment(workspaceId)}/companies/${segment(companyId)}/conversations/${segment(conversationId)}/messages`,{method:"POST",headers:{"x-csrf-token":csrf},body:JSON.stringify({content,idempotencyKey})})),
