...
import { WebChatConnectionNotFoundError, WebChatConnectionService } from "../webChat/services/webChatConnectionService.js";
...
test("EPIC-016.4 creates multiple opaque Company-bound Web Chat Connections and resolves active public IDs", async () => {
  const { database, primary, first, firstProfile, repository, service } = setup();
  try {
    const firstConnection = await service.create(primary, first.id, { assistantProfileId: firstProfile.id });
    const secondConnection = await service.create(primary, first.id, { assistantProfileId: firstProfile.id });
    ...
    assert.deepEqual((await service.list(primary, first.id)).map(({ id }) => id), [secondConnection.id, firstConnection.id]);
    assert.equal((await service.get(primary, first.id, firstConnection.id)).assistantProfileId, firstProfile.id);
    assert.equal((await service.resolveActiveByPublicId(firstConnection.publicId))?.id, firstConnection.id);
...
test("EPIC-016.4 transitions status idempotently and prevents inactive public resolution", async () => {
  const { database, primary, first, firstProfile, service } = setup();
  try {
    const connection = await service.create(primary, first.id, { assistantProfileId: firstProfile.id });
    const inactive = await service.setStatus(primary, first.id, connection.id, { status: "inactive" });
    assert.equal(inactive.status, "inactive"); assert.equal(await service.resolveActiveByPublicId(connection.publicId), null);
    assert.equal((await service.setStatus(primary, first.id, connection.id, { status: "inactive" })).id, connection.id);
    assert.equal((await service.setStatus(primary, first.id, connection.id, { status: "active" })).status, "active");
  } finally { database.close(); }
});

test("EPIC-016.4 enforces Workspace, Company, Profile, and Connection ownership", async () => {
  const { database, primary, secondary, first, second, foreign, firstProfile, secondProfile, foreignProfile, service } = setup();
  try {
    const connection = await service.create(primary, first.id, { assistantProfileId: firstProfile.id });
    await assert.rejects(() => service.create(primary, first.id, { assistantProfileId: secondProfile.id }), WebChatConnectionNotFoundError);
    await assert.rejects(() => service.create(primary, first.id, { assistantProfileId: foreignProfile.id }), WebChatConnectionNotFoundError);
    await assert.rejects(() => service.create(primary, foreign.id, { assistantProfileId: foreignProfile.id }), WebChatConnectionNotFoundError);
    await assert.rejects(() => service.get(secondary, first.id, connection.id), WebChatConnectionNotFoundError);
    await assert.rejects(() => service.setStatus(primary, second.id, connection.id, { status: "inactive" }), WebChatConnectionNotFoundError);
    await assert.rejects(() => service.create(primary, 999, { assistantProfileId: firstProfile.id }), WebChatConnectionNotFoundError);
  } finally { database.close(); }
});
