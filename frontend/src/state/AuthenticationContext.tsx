...
type BootstrapFlight = { readonly promise: Promise<SessionBootstrapResponse> };
let bootstrapFlight: BootstrapFlight | null = null;
function bootstrapSingleFlight(fresh = false): Promise<SessionBootstrapResponse> { if (!fresh && bootstrapFlight) return bootstrapFlight.promise; const flight: BootstrapFlight = { promise: atlasApi.bootstrapSession().finally(() => { if (bootstrapFlight === flight) bootstrapFlight = null; }) }; bootstrapFlight = flight; return flight.promise; }
...
export function AuthenticationProvider({ children }: { readonly children: ReactNode }): React.JSX.Element {
  const { locale } = useI18n(); const [state, dispatch] = useReducer(authenticationReducer, { status: "booting" }); const epoch = useRef(0); const bootstrapGeneration = useRef(0); const logoutInProgress = useRef(false); const channel = useRef<BroadcastChannel | null>(null); const sessionIncarnation = useRef<string | null>(storedSessionIncarnation()); const logoutFlight = useRef<Promise<void> | null>(null);
  ...
  const restoreSession = async (retryConflict = true, fresh = false): Promise<SessionBootstrapResponse | null> => {
    const currentEpoch = epoch.current, requestGeneration = ++bootstrapGeneration.current;
    try { const result = await bootstrapSingleFlight(fresh); if (epoch.current !== currentEpoch || bootstrapGeneration.current !== requestGeneration) return null; return result; }
    catch (cause: unknown) { if (epoch.current !== currentEpoch || bootstrapGeneration.current !== requestGeneration) return null; if (cause instanceof ApiError && cause.status === 409 && retryConflict) return restoreSession(false, fresh); if (cause instanceof ApiError && (cause.status === 401 || cause.status === 403)) { dispatch({ type: "unauthenticated", ...(cause.status === 403 ? { error: genericError() } : {}) }); return null; } dispatch({ type: "retryable", error: genericError() }); return null; }
  };
  const bootstrap = async (): Promise<boolean> => { if (logoutInProgress.current) return false; const result = await restoreSession(); if (!result) return false; dispatch({ type: "authenticated", result }); publishRotation(result); return true; };
  ...
  const logout = (): Promise<void> => {
    ...
    const execute = async (): Promise<void> => {
      ...
      try { await atlasApi.logout(current.csrfToken); }
        ...
        const restored = await restoreSession(true, true);
    ...
    logoutFlight.current = execute().finally(() => { logoutInProgress.current = false; logoutFlight.current = null; });
