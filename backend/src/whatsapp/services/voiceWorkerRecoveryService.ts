...
export class VoiceWorkerRecoveryService {
  ...
  public async transcribeAvailable(): Promise<void> { await this.each(worker => this.transcription.runOnce(worker.context, worker.companyId)); }
  public async synthesizeAvailable(): Promise<void> { await this.each(worker => this.synthesis.runOnce(worker.context, worker.companyId)); }
  public async uploadAvailable(): Promise<void> { await this.each(worker => this.upload.runOnce(worker.context, worker.companyId)); }

  private async each(run: (scope: { readonly context: { readonly workspaceId: number; readonly workspaceKey: string }; readonly companyId: number }) => Promise<unknown>): Promise<void> {
    for (const scope of await this.voices.recoverableVoiceWorkScopes(100)) await run({ context: { workspaceId: scope.workspaceId, workspaceKey: "whatsapp" }, companyId: scope.companyId });
  }
}
