...
export function CompanySummary({ company, onUpdate, onDelete }: Props): React.JSX.Element {
  ...
  const submit = async (event: FormEvent): Promise<void> => {
    ...
    try { await onUpdate({ ...form, website: form.website.trim() || null }); setEditing(false); setMessage({ type: "success", text: t("companies.updateSuccess") }); }
  ...
  return (
    <section className="workspace-section company-summary" aria-labelledby="company-title">
      <div className="workspace-title-row"><div><h1 id="company-title">{company.name}</h1>{company.website && <a href={company.website} target="_blank" rel="noreferrer">{company.website}<span className="sr-only"> ({t("accessibility.externalLink")})</span></a>}</div><StatusBadge status={company.status} /></div>
      {!editing && <>
