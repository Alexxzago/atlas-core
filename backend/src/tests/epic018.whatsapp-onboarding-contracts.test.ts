...
test("EPIC-018 queues a legacy safe fallback through the durable outbound service", async () => {
  ...
  assert.deepEqual(queued, [[{ workspaceId: 1, workspaceKey: "whatsapp" }, 7, { conversationId: "conversation", conversationMessageId: "cmsg_1123456789abcdef0123456789abcdef", whatsAppConnectionId: connection.id, recipientWaId: "wa" }]]);
...
test("EPIC-049 derives a safe credential source and rejects manual writes for Meta-managed connections", async () => {
  ...
  let legacyWrites = 0;
  const credentials = { findCredentials: () => null, replaceCredentials: () => null };
  const service = new WhatsAppConnectionService(
    { findById: () => ({ id: 7 }) } as never,
    {} as never,
    { findById: () => connection, findByIdForRecovery: () => connection, findByPhoneNumberId: () => null, listByCompany: () => [connection], replaceCredentialsAndDeactivate: () => { legacyWrites += 1; return connection; } } as never,
    { now: () => now },
