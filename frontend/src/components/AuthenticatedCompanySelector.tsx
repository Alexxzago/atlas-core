...
export function AuthenticatedCompanySelector(props: Props): React.JSX.Element | null {
  ...
  return <div className="company-chooser-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) props.onClose(); }}>
    <div ref={dialog} className="company-chooser" role="dialog" aria-modal="true" aria-labelledby="company-chooser-title">
      <header className="company-chooser__header"><div className="company-chooser__heading"><p className="company-chooser__eyebrow eyebrow">{t("companyChooser.eyebrow")}</p><h2 className="company-chooser__title" id="company-chooser-title">{props.companies.length === 0 ? t("companyChooser.firstTitle") : t("companyChooser.title")}</h2><p className="company-chooser__subtitle">{props.companies.length === 0 ? t("companyChooser.firstDescription") : t("companyChooser.description")}</p></div><button ref={closeButton} className="company-chooser__close atlas-icon-button" type="button" aria-label={t("common.close")} onClick={props.onClose}>×</button></header>
